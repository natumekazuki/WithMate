...
export function buildCharacterUpdateInstructionText(characterName: string): string {
  return [
    ...
    "- character.png は対になる代表アイコン画像として扱う",
    ...
    "- 画像を更新する場合は character.png を代表画像として扱い、採用理由や出典は character-notes.md に残す",
    "",
    "## 自己チェック",
    "",
...
export function buildCharacterUpdateSkillMarkdown(): string {
  return [
    ...
    "- character.png を、キャラクターの識別に使う代表画像として整える",
    "",
    "## 役割分担",
    "",
    "- character.md",
    ...
    "  - prompt の直接入力にはしない",
    "- character.png",
    "  - character.md と対になる代表画像",
    "  - 既存画像が妥当なら維持し、必要な時だけ更新する",
    "",
    "## 基本ルール",
    "",
    ...
    "- 画像を更新した場合も、何を根拠に採用したかを character-notes.md に残す",
    "",
    "## 更新手順",
    "",
    ...
    "5. 必要なら character.png を取得または更新し、workspace 内に保存する。",
    "6. character-notes.md に根拠、保留、競合解釈、改稿メモを残す。",
    "7. 変更後は更新内容と未確定事項を短く要約する。",
    "",
    ...
    "- character.png の相対パスと画像埋め込みを前提にしても、本文自体の構造が崩れないこと",
    "",
    "## 画像方針",
    "",
    "- 既存の character.png がそのまま使えるなら維持する",
    "- 画像が無い、古い、識別性が低い場合だけ更新する",
    "- 公式画像や一次情報に近い画像が使えるなら優先する",
    "- 難しい場合は広く認知された代表画像を使ってよいが、採用理由を notes に残す",
    "- 新しく保存する画像は workspace 内の character.png を正本として扱う",
    "",
    "## 自己チェック",
    "",
    ...
    "- 弱い根拠だけで中核定義を変えていないか",
    "- character.png を更新したなら、代表画像として妥当かを確認したか",
  ].join("\n");
}
...
export function buildCharacterMarkdownTemplate(characterName: string): string {
  return [
    ...
    `![${characterName} icon](./character.png)`,
    "",
    "## Core Persona",
    "- 中核となる価値観",
    "- 動機",
