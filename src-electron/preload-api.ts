...
function createSessionApi(
  ...
  return {
    ...
    listRelatedSessionSummaries(sessionIds) {
      return ipcRenderer.invoke(WITHMATE_LIST_RELATED_SESSION_SUMMARIES_CHANNEL, sessionIds);
    },
    listSessionCharacterUsage() {
      return ipcRenderer.invoke(WITHMATE_LIST_SESSION_CHARACTER_USAGE_CHANNEL);
    },
    ...
    listSessionTurnExecutions(sessionId) {
      return ipcRenderer.invoke(WITHMATE_LIST_SESSION_TURN_EXECUTIONS_CHANNEL, sessionId);
    },
...
function createSettingsApi(ipcRenderer: IpcRendererLike): WithMateWindowSettingsApi {
  return {
    ...
    getResourceBudget(input) {
      return ipcRenderer.invoke(WITHMATE_GET_RESOURCE_BUDGET_CHANNEL, input);
    },
    listResourceBudgets(input) {
      return ipcRenderer.invoke(WITHMATE_LIST_RESOURCE_BUDGETS_CHANNEL, input);
    },
    configureResourceBudget(input) {
      return ipcRenderer.invoke(WITHMATE_CONFIGURE_RESOURCE_BUDGET_CHANNEL, input);
    },
