...
export class AuditLogStorageV3 {
  ...
  async listSessionAuditLogs(sessionId: string): Promise<AuditLogEntry[]> {
    const { rows, operationRows } = this.withDb((db) => ({
      rows: db.prepare(LIST_SESSION_AUDIT_LOGS_SQL).all(sessionId) as AuditLogDetailRow[],
      operationRows: db.prepare(LIST_SESSION_AUDIT_LOG_OPERATIONS_SQL).all(sessionId) as AuditLogOperationRow[],
    }));
    ...
    for (const row of rows) {
      entries.push(await this.rowToAuditLogEntry(row, operationMap.get(row.id) ?? []));
    }
    return entries;
  }
  ...
  async getSessionAuditLogDetailSection(
    ...
    const result = this.withDb((db) => {
      ...
      const operationRows = section === "operations"
        ? db.prepare(LIST_AUDIT_LOG_OPERATIONS_SQL).all(auditLogId) as AuditLogOperationRow[]
        : [];
      return { row, operationRows };
    });
