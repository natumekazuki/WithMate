...
const workItemResultInputSchema = z.object({
  ...
  expectedResultRevision: z.number().int().min(1).optional(),
  result: workItemResultBodySchema,
  idempotencyKey: nonEmptyStringSchema,
}).strict();
const workItemResultCorrectionInputSchema = z.object({
  workItemId: nonEmptyStringSchema,
  expectedRevision: z.number().int().min(1),
  expectedResultRevision: z.number().int().min(1),
  correctionReason: nonEmptyStringSchema.max(WORK_ITEM_MAX_TEXT_LENGTH),
  result: workItemResultBodySchema.extend({ outcome: z.enum(["completed", "partially_completed", "failed"]) }).strict(),
  idempotencyKey: nonEmptyStringSchema,
}).strict();
...
const workItemAggregationListInputSchema = z.object({
  ...
  fields: z.array(z.enum(["summary", "decision", "provenance"])).min(1).max(3).refine((value) => new Set(value).size === value.length).optional(),
...
const workItemAggregationCorrectionInputSchema = z.object({
  parentWorkItemId: nonEmptyStringSchema,
...
const workItemIdentityShape = {
  ...
  resultCurrent: z.boolean().optional(),
...
const workItemAggregationItemSchema = z.object({
  ...
  depth: z.number().int().positive().max(8),
  provenance: z.object({ creatorSessionId: z.string(), targetSessionId: z.string(), parentWorkItemId: z.string().nullable() }).strict().optional(),
  resultRevision: z.number().int().nonnegative(), resultCurrent: z.boolean(), stale: z.boolean(),
...
const resultSchemas: Record<SessionRuntimeOperation, z.ZodType> = {
  ...
  "work.result.correct": z.object({
    workItem: workItemSchema,
    resultRevision: z.number().int().positive(),
    supersededResultRevision: z.number().int().positive(),
    stale: z.boolean(),
    staleParentWorkItemIds: z.array(z.string()),
  }).strict(),
  "work.cancel": canceledWorkItemSchema,
  "work.aggregation.get": z.object({
    contractRevision: z.literal(WORK_ITEM_AGGREGATION_CONTRACT_REVISION), parentWorkItemId: z.string(), aggregateRevision: z.number().int().nonnegative(),
    ...
    stale: z.boolean().optional(), staleReasons: z.array(z.string()).optional(), finalizedRevision: z.number().int().positive().nullable().optional(),
    finalizedResultRevision: z.number().int().positive().nullable().optional(),
  }).strict(),
  ...
  "work.aggregation.correct": z.object({
    decision: workItemAggregationDecisionSchema.nullable(),
    supersededDecisionRevision: z.number().int().positive(),
