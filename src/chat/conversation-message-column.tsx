...
export type UseConversationMessageColumnInput = {
  ...
  runningProjectionBarrierExecutionId?: string | null;
...
export function useConversationMessageColumn({
  session, baseProps, enabled, api, liveRun: liveRunOverride, stateCache, runningProjectionBarrierExecutionId, onColumnControls,
  ...
  const turnExecutions = useMemo(
    () => (baseProps.turnExecutions ?? []).filter(
      (execution): execution is SessionTurnExecutionProjection => execution !== null,
    ),
    [baseProps.turnExecutions],
  );
  const projectedMessageList = useMemo(
    () => appendTurnExecutionsToMessageList(
      projection,
      turnExecutions,
      session?.runState ?? (baseProps.isRunning ? "running" : "idle"),
      runningProjectionBarrierExecutionId ?? null,
    ),
    [baseProps.isRunning, projection, runningProjectionBarrierExecutionId, session?.runState, turnExecutions],
  );
  const messageScrollSignature = useMemo(
    () => `${projectedMessageList.keys.join("\u001f")}:${projectedMessageList.messages.map((message) => message.text.length).join(",")}`,
    [projectedMessageList],
  );
  const previousTargets = useRef<SessionMessageColumnProps["messageCollapseTargets"]>([]);
  const collapseTargets = useMemo(() => buildMessageCollapseTargets(projectedMessageList.messages, projectedMessageList.sources, projectedMessageList.keys, previousTargets.current),
    [projectedMessageList]);
  ...
  return {
    ...
    messages: projectedMessageList.messages,
    messageKeys: projectedMessageList.keys,
    messageGroups: projectedMessageList.groups,
    turnExecutions: projectedMessageList.turnExecutions,
    ...
    onLoadArtifactDetail: baseProps.onLoadArtifactDetail
      ? (messageIndex) => loadProjectedMessageArtifact({
        source: projectedMessageList.sources[messageIndex],
