...
export type ConcurrentChatWindowProps = {
  ...
  loading?: boolean;
  error?: string | null;
  mainRunningProjectionBarrierExecutionId?: string | null;
};
...
export function ChatWindow({
  ...
  const allowTargetChange = concurrentChats?.allowTargetChange !== false;
  ...
  useShortcutCommandHandler(
    ...
    Boolean(concurrentChats && allowTargetChange),
  );
  ...
  const targetColumnControls = rawTargetColumnControls?.sessionId === targetSessionId
    ...
    : null;
  const targetComposerSend = concurrentChats && allowTargetChange && targetColumnControls
    ? () => {
  ...
  const resolvedRightPaneProps = concurrentChats && screenProps.rightPaneProps
    ...
    : screenProps.rightPaneProps;
  const targetDock = concurrentChats && allowTargetChange
    ? <ConcurrentChatTargetDock chats={concurrentChats} />
    : null;
        ...
        <div id="session-main-chat-pane" className="concurrent-chat-column-content">
          {concurrentChats ? (
            <ConversationMessageColumn
              ...
              runningProjectionBarrierExecutionId={concurrentChats.mainRunningProjectionBarrierExecutionId}
