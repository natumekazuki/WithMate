export const WITHMATE_OPEN_SESSION_CHANNEL = "withmate:open-session";
export const WITHMATE_GET_SESSION_WINDOW_RESTORE_SET_CHANNEL = "withmate:get-session-window-restore-set";
export const WITHMATE_RESTORE_SESSION_WINDOWS_CHANNEL = "withmate:restore-session-windows";
export const WITHMATE_OPEN_HOME_WINDOW_CHANNEL = "withmate:open-home-window";
export const WITHMATE_OPEN_SESSION_MONITOR_WINDOW_CHANNEL = "withmate:open-session-monitor-window";
export const WITHMATE_OPEN_SETTINGS_WINDOW_CHANNEL = "withmate:open-settings-window";
export const WITHMATE_OPEN_MEMORY_V6_REVIEW_WINDOW_CHANNEL = "withmate:open-memory-v6-review-window";
export const WITHMATE_OPEN_COORDINATION_WINDOW_CHANNEL = "withmate:open-coordination-window";
export const WITHMATE_OPEN_CHARACTER_EDITOR_WINDOW_CHANNEL = "withmate:open-character-editor-window";
export const WITHMATE_OPEN_DIFF_WINDOW_CHANNEL = "withmate:open-diff-window";
export const WITHMATE_OPEN_COMPANION_REVIEW_WINDOW_CHANNEL = "withmate:open-companion-review-window";
export const WITHMATE_OPEN_COMPANION_MERGE_WINDOW_CHANNEL = "withmate:open-companion-merge-window";
export const WITHMATE_LIST_SESSION_SUMMARY_PAGE_CHANNEL = "withmate:list-session-summary-page";
export const WITHMATE_LIST_RELATED_SESSION_SUMMARIES_CHANNEL = "withmate:list-related-session-summaries";
export const WITHMATE_LIST_SESSION_CHARACTER_USAGE_CHANNEL = "withmate:list-session-character-usage";
export const WITHMATE_GET_SESSION_CHANNEL = "withmate:get-session";
export const WITHMATE_GET_ROOT_WORK_ITEM_CHANNEL = "withmate:get-root-work-item";
export const WITHMATE_LIST_ROOT_WORK_ITEM_HISTORY_CHANNEL = "withmate:list-root-work-item-history";
export const WITHMATE_REVISE_ROOT_WORK_ITEM_CHANNEL = "withmate:revise-root-work-item";
export const WITHMATE_APPEND_ROOT_WORK_ITEM_HISTORY_CHANNEL = "withmate:append-root-work-item-history";
export const WITHMATE_GET_SESSION_GLOSSARY_PROJECTION_CHANNEL = "withmate:get-session-glossary-projection";
export const WITHMATE_SEARCH_SESSION_GLOSSARY_CHANNEL = "withmate:search-session-glossary";
export const WITHMATE_VALIDATE_SESSION_WORKSPACE_CHANNEL = "withmate:validate-session-workspace";
export const WITHMATE_LIST_SESSION_FILE_ROOTS_CHANNEL = "withmate:list-session-file-roots";
export const WITHMATE_LIST_SESSION_DIRECTORY_CHANNEL = "withmate:list-session-directory";
export const WITHMATE_INSPECT_SESSION_FILE_CHANNEL = "withmate:inspect-session-file";
export const WITHMATE_READ_SESSION_FILE_CHUNK_CHANNEL = "withmate:read-session-file-chunk";
export const WITHMATE_OPEN_SESSION_FILE_CHANNEL = "withmate:open-session-file";
export const WITHMATE_OPEN_SESSION_FILE_PREVIEW_WINDOW_CHANNEL = "withmate:open-session-file-preview-window";
export const WITHMATE_GET_SESSION_FILE_PREVIEW_WINDOW_PAYLOAD_CHANNEL =
  "withmate:get-session-file-preview-window-payload";
export const WITHMATE_SESSION_FILE_PREVIEW_NAVIGATION_EVENT =
  "withmate:session-file-preview-navigation";
export const WITHMATE_COPY_SESSION_FILE_PREVIEW_IMAGE_CHANNEL = "withmate:copy-session-file-preview-image";
export const WITHMATE_SHOW_SESSION_FILE_PREVIEW_IMAGE_CONTEXT_MENU_CHANNEL =
  "withmate:show-session-file-preview-image-context-menu";
export const WITHMATE_COPY_SESSION_FILE_OBJECT_CHANNEL = "withmate:copy-session-file-object";
export const WITHMATE_SHOW_SESSION_FILE_OBJECT_COPY_CONTEXT_MENU_CHANNEL =
  "withmate:show-session-file-object-copy-context-menu";
export const WITHMATE_SHOW_SESSION_FILE_TREE_CONTEXT_MENU_CHANNEL =
  "withmate:show-session-file-tree-context-menu";
export const WITHMATE_SHOW_MARKDOWN_LINK_CONTEXT_MENU_CHANNEL = "withmate:show-markdown-link-context-menu";
export const WITHMATE_LIST_FILE_ROOT_CHANGES_CHANNEL = "withmate:list-file-root-changes";
export const WITHMATE_LIST_FILE_ROOT_CHANGES_REPOSITORIES_CHANNEL =
  "withmate:list-file-root-changes-repositories";
export const WITHMATE_GET_FILE_ROOT_DIFF_CHANNEL = "withmate:get-file-root-diff";
export const WITHMATE_LIST_FILE_ROOT_GIT_HISTORY_REPOSITORIES_CHANNEL =
  "withmate:list-file-root-git-history-repositories";
export const WITHMATE_LIST_FILE_ROOT_GIT_HISTORY_COMMITS_CHANNEL =
  "withmate:list-file-root-git-history-commits";
export const WITHMATE_GET_FILE_ROOT_GIT_HISTORY_COMMIT_DETAIL_CHANNEL =
  "withmate:get-file-root-git-history-commit-detail";
export const WITHMATE_GET_FILE_ROOT_GIT_HISTORY_DIFF_CHANNEL =
  "withmate:get-file-root-git-history-diff";
export const WITHMATE_GET_SESSION_MESSAGE_ARTIFACT_CHANNEL = "withmate:get-session-message-artifact";
export const WITHMATE_GET_MODEL_CATALOG_CHANNEL = "withmate:get-model-catalog";
export const WITHMATE_IMPORT_MODEL_CATALOG_CHANNEL = "withmate:import-model-catalog";
export const WITHMATE_EXPORT_MODEL_CATALOG_CHANNEL = "withmate:export-model-catalog";
export const WITHMATE_IMPORT_MODEL_CATALOG_FILE_CHANNEL = "withmate:import-model-catalog-file";
export const WITHMATE_EXPORT_MODEL_CATALOG_FILE_CHANNEL = "withmate:export-model-catalog-file";
export const WITHMATE_GET_DIFF_PREVIEW_CHANNEL = "withmate:get-diff-preview";
export const WITHMATE_LIST_AUXILIARY_SESSIONS_CHANNEL = "withmate:list-auxiliary-sessions";
export const WITHMATE_LIST_OPEN_ACTIVE_AUXILIARY_SESSION_SUMMARIES_CHANNEL =
  "withmate:list-open-active-auxiliary-session-summaries";
export const WITHMATE_GET_ACTIVE_AUXILIARY_SESSION_CHANNEL = "withmate:get-active-auxiliary-session";
export const WITHMATE_GET_AUXILIARY_SESSION_CHANNEL = "withmate:get-auxiliary-session";
export const WITHMATE_CREATE_AUXILIARY_SESSION_CHANNEL = "withmate:create-auxiliary-session";
export const WITHMATE_UPDATE_AUXILIARY_SESSION_CHANNEL = "withmate:update-auxiliary-session";
export const WITHMATE_CLOSE_AUXILIARY_SESSION_CHANNEL = "withmate:close-auxiliary-session";
export const WITHMATE_RUN_AUXILIARY_SESSION_TURN_CHANNEL = "withmate:run-auxiliary-session-turn";
export const WITHMATE_CANCEL_AUXILIARY_SESSION_RUN_CHANNEL = "withmate:cancel-auxiliary-session-run";
export const WITHMATE_CREATE_SESSION_CHANNEL = "withmate:create-session";
export const WITHMATE_CREATE_COMPANION_SESSION_CHANNEL = "withmate:create-companion-session";
export const WITHMATE_LIST_COMPANION_SESSION_SUMMARIES_CHANNEL = "withmate:list-companion-session-summaries";
export const WITHMATE_GET_COMPANION_SESSION_CHANNEL = "withmate:get-companion-session";
export const WITHMATE_GET_COMPANION_MESSAGE_ARTIFACT_CHANNEL = "withmate:get-companion-message-artifact";
export const WITHMATE_GET_COMPANION_REVIEW_SNAPSHOT_CHANNEL = "withmate:get-companion-review-snapshot";
export const WITHMATE_MERGE_COMPANION_SELECTED_FILES_CHANNEL = "withmate:merge-companion-selected-files";
export const WITHMATE_SYNC_COMPANION_TARGET_CHANNEL = "withmate:sync-companion-target";
export const WITHMATE_STASH_COMPANION_TARGET_CHANGES_CHANNEL = "withmate:stash-companion-target-changes";
export const WITHMATE_RESTORE_COMPANION_TARGET_STASH_CHANNEL = "withmate:restore-companion-target-stash";
export const WITHMATE_DROP_COMPANION_TARGET_STASH_CHANNEL = "withmate:drop-companion-target-stash";
export const WITHMATE_DISCARD_COMPANION_SESSION_CHANNEL = "withmate:discard-companion-session";
export const WITHMATE_RUN_COMPANION_SESSION_TURN_CHANNEL = "withmate:run-companion-session-turn";
export const WITHMATE_CANCEL_COMPANION_SESSION_RUN_CHANNEL = "withmate:cancel-companion-session-run";
export const WITHMATE_UPDATE_COMPANION_SESSION_CHANNEL = "withmate:update-companion-session";
export const WITHMATE_PREVIEW_COMPANION_COMPOSER_INPUT_CHANNEL = "withmate:preview-companion-composer-input";
export const WITHMATE_UPDATE_SESSION_CHANNEL = "withmate:update-session";
export const WITHMATE_SET_SESSION_PINNED_CHANNEL = "withmate:set-session-pinned";
export const WITHMATE_DELETE_SESSION_CHANNEL = "withmate:delete-session";
export const WITHMATE_DELETE_SESSIONS_LAST_ACTIVE_BEFORE_CHANNEL = "withmate:delete-sessions-last-active-before";
export const WITHMATE_GET_MATE_STATE_CHANNEL = "withmate:get-mate-state";
export const WITHMATE_GET_MATE_PROFILE_CHANNEL = "withmate:get-mate-profile";
export const WITHMATE_CREATE_MATE_CHANNEL = "withmate:create-mate";
export const WITHMATE_UPDATE_MATE_CHANNEL = "withmate:update-mate";
export const WITHMATE_SET_MATE_AVATAR_CHANNEL = "withmate:set-mate-avatar";
export const WITHMATE_RESET_MATE_CHANNEL = "withmate:reset-mate";
export const WITHMATE_LIST_CHARACTERS_CHANNEL = "withmate:list-characters";
export const WITHMATE_GET_CHARACTER_CHANNEL = "withmate:get-character";
export const WITHMATE_CREATE_CHARACTER_CHANNEL = "withmate:create-character";
export const WITHMATE_UPDATE_CHARACTER_METADATA_CHANNEL = "withmate:update-character-metadata";
export const WITHMATE_UPDATE_CHARACTER_DEFINITION_CHANNEL = "withmate:update-character-definition";
export const WITHMATE_ARCHIVE_CHARACTER_CHANNEL = "withmate:archive-character";
export const WITHMATE_RESOLVE_LAUNCH_CHARACTER_CHANNEL = "withmate:resolve-launch-character";
export const WITHMATE_START_CHARACTER_AUTHORING_SESSION_CHANNEL = "withmate:start-character-authoring-session";
export const WITHMATE_PICK_DIRECTORY_CHANNEL = "withmate:pick-directory";
export const WITHMATE_VALIDATE_WORKSPACE_DIRECTORY_CHANNEL = "withmate:validate-workspace-directory";
export const WITHMATE_PICK_FILE_CHANNEL = "withmate:pick-file";
export const WITHMATE_PICK_FILES_CHANNEL = "withmate:pick-files";
export const WITHMATE_PICK_SESSION_FILES_CHANNEL = "withmate:pick-session-files";
export const WITHMATE_PICK_SESSION_FOLDER_CHANNEL = "withmate:pick-session-folder";
export const WITHMATE_PICK_SESSION_IMAGE_FILE_CHANNEL = "withmate:pick-session-image-file";
export const WITHMATE_PICK_IMAGE_FILE_CHANNEL = "withmate:pick-image-file";
export const WITHMATE_COPY_FILES_TO_SESSION_FILES_CHANNEL = "withmate:copy-files-to-session-files";
export const WITHMATE_SAVE_PASTED_SESSION_FILE_CHANNEL = "withmate:save-pasted-session-file";
export const WITHMATE_OPEN_SESSION_FILES_DIRECTORY_CHANNEL = "withmate:open-session-files-directory";
export const WITHMATE_OPEN_SESSION_FILES_TERMINAL_CHANNEL = "withmate:open-session-files-terminal";
export const WITHMATE_OPEN_PATH_CHANNEL = "withmate:open-path";
export const WITHMATE_OPEN_APP_LOG_FOLDER_CHANNEL = "withmate:open-app-log-folder";
export const WITHMATE_OPEN_CRASH_DUMP_FOLDER_CHANNEL = "withmate:open-crash-dump-folder";
export const WITHMATE_OPEN_SESSION_TERMINAL_CHANNEL = "withmate:open-session-terminal";
export const WITHMATE_OPEN_TERMINAL_AT_PATH_CHANNEL = "withmate:open-terminal-at-path";
export const WITHMATE_RENDERER_LOG_CHANNEL = "withmate:renderer-log";
export const WITHMATE_PREVIEW_COMPOSER_INPUT_CHANNEL = "withmate:preview-composer-input";
export const WITHMATE_LIST_SESSION_SKILLS_CHANNEL = "withmate:list-session-skills";
export const WITHMATE_LIST_SESSION_CUSTOM_AGENTS_CHANNEL = "withmate:list-session-custom-agents";
export const WITHMATE_LIST_WORKSPACE_SKILLS_CHANNEL = "withmate:list-workspace-skills";
export const WITHMATE_LIST_WORKSPACE_CUSTOM_AGENTS_CHANNEL = "withmate:list-workspace-custom-agents";
export const WITHMATE_RUN_SESSION_TURN_CHANNEL = "withmate:run-session-turn";
export const WITHMATE_ENQUEUE_SESSION_TURN_CHANNEL = "withmate:enqueue-session-turn";
export const WITHMATE_LIST_SESSION_TURN_EXECUTIONS_CHANNEL = "withmate:list-session-turn-executions";
export const WITHMATE_CANCEL_SESSION_EXECUTION_CHANNEL = "withmate:cancel-session-execution";
export const WITHMATE_CANCEL_SESSION_RUN_CHANNEL = "withmate:cancel-session-run";
export const WITHMATE_LIST_COORDINATION_EVENTS_CHANNEL = "withmate:list-coordination-events";
export const WITHMATE_GET_COORDINATION_EVENT_CHANNEL = "withmate:get-coordination-event";
export const WITHMATE_RESOLVE_COORDINATION_EVENT_CHANNEL = "withmate:resolve-coordination-event";
export const WITHMATE_CANCEL_COORDINATION_EVENT_CHANNEL = "withmate:cancel-coordination-event";
export const WITHMATE_LIST_SESSION_SCHEDULES_CHANNEL = "withmate:list-session-schedules";
export const WITHMATE_GET_SESSION_SCHEDULE_CHANNEL = "withmate:get-session-schedule";
export const WITHMATE_CREATE_SESSION_SCHEDULE_CHANNEL = "withmate:create-session-schedule";
export const WITHMATE_UPDATE_SESSION_SCHEDULE_CHANNEL = "withmate:update-session-schedule";
export const WITHMATE_PAUSE_SESSION_SCHEDULE_CHANNEL = "withmate:pause-session-schedule";
export const WITHMATE_RESUME_SESSION_SCHEDULE_CHANNEL = "withmate:resume-session-schedule";
export const WITHMATE_DELETE_SESSION_SCHEDULE_CHANNEL = "withmate:delete-session-schedule";
export const WITHMATE_RUN_SESSION_SCHEDULE_NOW_CHANNEL = "withmate:run-session-schedule-now";
export const WITHMATE_LIST_SESSION_AUDIT_LOGS_CHANNEL = "withmate:list-session-audit-logs";
export const WITHMATE_LIST_SESSION_AUDIT_LOG_SUMMARIES_CHANNEL = "withmate:list-session-audit-log-summaries";
export const WITHMATE_LIST_SESSION_AUDIT_LOG_SUMMARY_PAGE_CHANNEL = "withmate:list-session-audit-log-summary-page";
export const WITHMATE_GET_SESSION_AUDIT_LOG_DETAIL_CHANNEL = "withmate:get-session-audit-log-detail";
export const WITHMATE_GET_SESSION_AUDIT_LOG_DETAIL_SECTION_CHANNEL = "withmate:get-session-audit-log-detail-section";
export const WITHMATE_GET_SESSION_AUDIT_LOG_OPERATION_DETAIL_CHANNEL = "withmate:get-session-audit-log-operation-detail";
export const WITHMATE_LIST_COMPANION_AUDIT_LOGS_CHANNEL = "withmate:list-companion-audit-logs";
export const WITHMATE_LIST_COMPANION_AUDIT_LOG_SUMMARIES_CHANNEL = "withmate:list-companion-audit-log-summaries";
export const WITHMATE_LIST_COMPANION_AUDIT_LOG_SUMMARY_PAGE_CHANNEL = "withmate:list-companion-audit-log-summary-page";
export const WITHMATE_GET_COMPANION_AUDIT_LOG_DETAIL_CHANNEL = "withmate:get-companion-audit-log-detail";
export const WITHMATE_GET_COMPANION_AUDIT_LOG_DETAIL_SECTION_CHANNEL = "withmate:get-companion-audit-log-detail-section";
export const WITHMATE_GET_COMPANION_AUDIT_LOG_OPERATION_DETAIL_CHANNEL = "withmate:get-companion-audit-log-operation-detail";
export const WITHMATE_GET_APP_SETTINGS_CHANNEL = "withmate:get-app-settings";
export const WITHMATE_UPDATE_APP_SETTINGS_CHANNEL = "withmate:update-app-settings";
export const WITHMATE_UPDATE_CHAT_LAYOUT_PREFERENCE_CHANNEL = "withmate:update-chat-layout-preference";
export const WITHMATE_GET_RESOURCE_BUDGET_CHANNEL = "withmate:get-resource-budget";
export const WITHMATE_LIST_RESOURCE_BUDGETS_CHANNEL = "withmate:list-resource-budgets";
export const WITHMATE_CONFIGURE_RESOURCE_BUDGET_CHANNEL = "withmate:configure-resource-budget";
export const WITHMATE_LIST_PROMPT_TEMPLATES_CHANNEL = "withmate:list-prompt-templates";
export const WITHMATE_CREATE_PROMPT_TEMPLATE_CHANNEL = "withmate:create-prompt-template";
export const WITHMATE_UPDATE_PROMPT_TEMPLATE_CHANNEL = "withmate:update-prompt-template";
export const WITHMATE_DELETE_PROMPT_TEMPLATE_CHANNEL = "withmate:delete-prompt-template";
export const WITHMATE_GET_APP_DATABASE_DIAGNOSTICS_CHANNEL = "withmate:get-app-database-diagnostics";
export const WITHMATE_GET_MEMORY_V6_DIAGNOSTICS_CHANNEL = "withmate:get-memory-v6-diagnostics";
export const WITHMATE_GET_SESSION_INTEGRATION_DIAGNOSTICS_CHANNEL =
  "withmate:get-session-integration-diagnostics";
export const WITHMATE_REGISTER_CODEX_SESSION_MCP_CHANNEL = "withmate:register-codex-session-mcp";
export const WITHMATE_INSTALL_MEMORY_V6_CLI_SHIM_CHANNEL = "withmate:install-memory-v6-cli-shim";
export const WITHMATE_UNINSTALL_MEMORY_V6_CLI_SHIM_CHANNEL = "withmate:uninstall-memory-v6-cli-shim";
export const WITHMATE_GET_MEMORY_V6_FILE_USAGE_CHANNEL = "withmate:get-memory-v6-file-usage";
export const WITHMATE_EXPORT_MEMORY_V6_ENTRY_FILES_CHANNEL = "withmate:export-memory-v6-entry-files";
export const WITHMATE_RUN_MEMORY_V6_PROTECTED_OBJECT_GC_CHANNEL = "withmate:run-memory-v6-protected-object-gc";
export const WITHMATE_SEARCH_MEMORY_V6_ENTRIES_CHANNEL = "withmate:search-memory-v6-entries";
export const WITHMATE_GET_MEMORY_V6_ENTRY_CHANNEL = "withmate:get-memory-v6-entry";
export const WITHMATE_FORGET_MEMORY_V6_ENTRY_CHANNEL = "withmate:forget-memory-v6-entry";
export const WITHMATE_GET_APP_BOOT_STATUS_CHANNEL = "withmate:get-app-boot-status";
export const WITHMATE_RESET_APP_DATABASE_CHANNEL = "withmate:reset-app-database";
export const WITHMATE_GET_LIVE_SESSION_RUN_CHANNEL = "withmate:get-live-session-run";
export const WITHMATE_GET_PROVIDER_QUOTA_TELEMETRY_CHANNEL = "withmate:get-provider-quota-telemetry";
export const WITHMATE_GET_SESSION_CONTEXT_TELEMETRY_CHANNEL = "withmate:get-session-context-telemetry";
export const WITHMATE_GET_SESSION_BACKGROUND_ACTIVITY_CHANNEL = "withmate:get-session-background-activity";
export const WITHMATE_RESOLVE_LIVE_APPROVAL_CHANNEL = "withmate:resolve-live-approval";
export const WITHMATE_RESOLVE_LIVE_ELICITATION_CHANNEL = "withmate:resolve-live-elicitation";
export const WITHMATE_LIST_OPEN_SESSION_WINDOW_IDS_CHANNEL = "withmate:list-open-session-window-ids";
export const WITHMATE_LIST_OPEN_COMPANION_REVIEW_WINDOW_IDS_CHANNEL = "withmate:list-open-companion-review-window-ids";
export const WITHMATE_SESSIONS_INVALIDATED_EVENT = "withmate:sessions-invalidated";
export const WITHMATE_SESSION_EXECUTIONS_CHANGED_EVENT = "withmate:session-executions-changed";
export const WITHMATE_COORDINATION_EVENTS_CHANGED_EVENT = "withmate:coordination-events-changed";
export const WITHMATE_SESSION_SCHEDULES_CHANGED_EVENT = "withmate:session-schedules-changed";
export const WITHMATE_MODEL_CATALOG_CHANGED_EVENT = "withmate:model-catalog-changed";
export const WITHMATE_APP_SETTINGS_CHANGED_EVENT = "withmate:app-settings-changed";
export const WITHMATE_PROMPT_TEMPLATES_CHANGED_EVENT = "withmate:prompt-templates-changed";
export const WITHMATE_LIVE_SESSION_RUN_EVENT = "withmate:live-session-run";
export const WITHMATE_PROVIDER_QUOTA_TELEMETRY_EVENT = "withmate:provider-quota-telemetry";
export const WITHMATE_SESSION_CONTEXT_TELEMETRY_EVENT = "withmate:session-context-telemetry";
export const WITHMATE_SESSION_BACKGROUND_ACTIVITY_EVENT = "withmate:session-background-activity";
export const WITHMATE_SESSION_GLOSSARY_CHANGED_EVENT = "withmate:session-glossary-changed";
export const WITHMATE_OPEN_SESSION_WINDOWS_CHANGED_EVENT = "withmate:open-session-windows-changed";
export const WITHMATE_SESSION_WINDOW_RESTORE_SET_CHANGED_EVENT = "withmate:session-window-restore-set-changed";
export const WITHMATE_OPEN_COMPANION_REVIEW_WINDOWS_CHANGED_EVENT = "withmate:open-companion-review-windows-changed";
export const WITHMATE_COMPANION_SESSIONS_CHANGED_EVENT = "withmate:companion-sessions-changed";
export const WITHMATE_APP_BOOT_STATUS_EVENT = "withmate:app-boot-status";
