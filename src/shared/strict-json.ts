...
export class StrictJsonError extends Error {
  constructor(readonly code: "invalid_json" | "duplicate_field") {
    super(code === "invalid_json" ? "JSON text is invalid." : "JSON text contains a duplicate object field.");
  ...
  while (offset < text.length) {
    ...
    switch (frame.state) {
      case "keyOrEnd": {
        ...
        if (frame.keys.has(key.value)) throw new StrictJsonError("duplicate_field");
...
function consumeValue(text: string, offset: number, frames: JsonFrame[]): number | undefined {
  ...
  if (character === '"') return readJsonString(text, offset)?.end;
  let end = offset;
