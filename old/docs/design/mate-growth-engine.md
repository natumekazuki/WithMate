...
- V5 Character Core / preview では Growth Candidate、MateTalk、provider instruction sync を current prompt route として扱わない。Character runtime snapshot と prompt boundary は `docs/design/character-storage.md` と `docs/design/prompt-composition.md` を優先する
...
Growth LLM provider / model / depth は `mate_growth_model_preferences` の `memory_candidate` row から選ぶ。
4.0.0 MVP の runtime は Memory Candidate 生成だけを Growth model preference の対象にし、`profile_update` / `project_digest` 用の自由な priority list は提供しない。
Settings は purpose を編集させず、provider は既知 provider、model は選択 provider の model catalog、depth は選択 model の reasoning effort から選ばせる。
...
`targetSection` は Memory Candidate 生成時に分類 hint として受け取り、app 側で保存前に正規化する。
V4 legacy MateTalk 由来の明示指示 / 訂正で Mate 自身の自己定義、一人称、性格、自己認識を扱う候補は `core` に補正する。
`policyDecision`、`projectionAllowed` は保存時は `pending` / `false` で初期化し、GrowthPolicyGate / PostPolicyGate が apply 前に決める。
...
V4 legacy の `mate-talk` は `canDisableFileWriteTools` と `canDisableShellWriteTools` が両方 true の provider だけで有効化する。V5 preview では legacy MateTalk runtime / window / chat mode を current runtime として有効化しない。
...
4.0.0 MVP の既定は、`memory_candidate_mode = 'every_turn'` の場合に Memory Candidate 生成を turn ごとに実行し、Growth apply / consolidation は elapsed time、pending Memory の有無、manual run を主条件にする。
...
`threshold` / `manual` は schema 上の予約値として保持するが、4.0.0 MVP では Settings の選択肢として出さず、自動 Memory Candidate 生成も抑制する。threshold evaluator や manual trigger を実装するまでは、保存済み値が残っていても runtime は no-op とする。
