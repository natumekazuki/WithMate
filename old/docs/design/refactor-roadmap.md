...
- `SessionWindowBridge`
  - 完了
  - `Session Window` の registry / close policy / session-start hook
...
- `IPC Boundary Test Hardening`
  - 完了
  - `preload-api` の public shape と payload unwrap を test で固定
  - `main-ipc-registration` の invoke channel registration を domain 横断で固定
- `Renderer WithMate API Helper Refactor`
  - 完了
  - `window.withmate` の取得と desktop runtime 判定を `renderer-withmate-api.ts` へ共通化
  - `DiffApp`、`CharacterEditorApp`、`HomeApp`、`App` の主要な直参照を helper 経由へ整理
- `Renderer IPC Wrapper Cleanup`
  - 完了
  - `withWithMateApi` helper を追加
  - `HomeApp` の open/pick/create wrapper と `CharacterEditorApp` の save/delete を最小 helper へ整理
- `WithMate Window API Surface Review`
  - 完了
  - `WithMateWindowApi` を domain interface に分割し、public surface の見通しを整理
  - `preload-api.ts` も同じ domain 単位の型で返り値を整理
- 次の候補
  - renderer helper と `withmate-window-api` の境界をこのまま維持するかの再確認
  - docs の current 正本を final 仕様へ再点検
