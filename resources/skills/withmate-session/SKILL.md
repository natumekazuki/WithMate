...
4. Materialize every planned child before dispatch: pass the current `session.self.revision` as `expectedContainerRevision` to `session.create` with a caller-owned idempotency key, then refresh `session.self` and use `session.get` to confirm the canonical child, workspace identity, and target revision.
