...
- Treat `runtime.catalog.authority.operations` as the canonical operation classification, not as proof that the current actor holds a grant. `baselineChildSessionRoleTemplates` describes grant issuance templates only; the runtime evaluates the actor's current saved grant for every request.
- New Work Item lifecycle capabilities are not added to existing baseline grants automatically. Only an actor explicitly granted the capability through a trusted root policy or grant.create within an existing parent ceiling may execute it. Do not reissue grants to expand access.
- Use `grant.create` only with an existing parent grant and current parent revision. Keep issued actions, resource IDs, budget, and expiry within the issuer ceiling; use `grant.revoke` with the current grant revision and retain the grant ID for consultation Turn provenance.
- Read `budget.get` before creating or dispatching resources when capacity may be tight. A soft-limit alert is guidance for the Agent; a hard-limit or deadline rejection blocks new effects. Use `budget.configure` only within the actor's existing allocation. Increasing a root hard limit or the per-execution retry limit requires trusted user or issuer authority and cannot be obtained through the Agent operation. Treat only main Session `turn.run` and `turn.enqueue` executions with canonical execution IDs as covered by the root Turn, retry, and generation ledger.
...
- Before `turn.run` or `turn.enqueue`, use the canonical bindings returned by `session.self` and `session.get` to select an allowed target. The Role-based baseline templates describe initial grants, not a live authority ceiling. Explicit communication grants may authorize other targets in the same root; associate non-self dispatch with a Work Item or consultation grant.
- Do not dispatch without an active grant covering the canonical target. Cross-root dispatch requires a bounded consultation grant. Do not send actor Role, root, parent, or depth in the request; WithMate derives them from runtime and canonical Role bindings and rejects unauthorized Turns before acceptance.
- For cross-root consultation, pass the issued `consultationGrantId` together with the explicit target and keep the grant's purpose, completion criteria, resource IDs, budget account, expiry, and return Session unchanged. A consultation grant provides temporary access and never transfers ownership.
...
- Derive permission for Session creation, Turn execution, cancellation, overwrite, and other external effects from the user's instruction. This Skill does not grant additional authority or permit an Agent to create a user answer, consent, or approval.
...
- Treat each child result as an adoption candidate. A task coordinator validates its direct children and closes them through existing aggregation decisions and its strict parent result. A root overall coordinator keeps its self-owned root Work Item current with `work.revise` and `work.history.append`, then validates and adopts each top-level result through `work.get` without inventing a parent aggregation.
...
4. Materialize every planned child before dispatch: pass the current `session.self.revision` as `expectedContainerRevision` to `session.create` with a caller-owned idempotency key, then refresh `session.self` and use `session.get` to confirm the canonical child, workspace identity, and target revision. When the child needs a dedicated allocation, call `budget.configure` on the parent account with `childAllocation.accountId`, the canonical `childSessionId`, and all eight `hardLimits`; set `storageBytes` to `0` because storage uses the shared root account, and do not supply owner, authority grant, or derived hierarchy fields.
5. Call `work.create` for every planned child with an operation-specific idempotency key and that target Session's current revision as `expectedContainerRevision`. A task coordinator includes its active current Work Item as `parentWorkItemId`; a root overall coordinator omits `parentWorkItemId` for top-level children. Refresh the target Session after creation. Do not dispatch until every planned child has a canonical Work Item.
6. For each dependency-ready child, call `turn.options`, construct a currently supported provider tuple, refresh the target with `session.get`, and dispatch with `turn.run` or `turn.enqueue` using its current revision as `expectedContainerRevision`, the Work Item ID, and a separate key. Put the exact Work Item ID in the delegation prompt and require the target to confirm it with `work.get` before `work.transition` or `work.result`; the execution association alone is not prompt context. Do not switch operations after an admission failure. Leave dependent Work Items undispatched until their prerequisite satisfies the decision rules above.
...
8. Require the target Session to submit an explicit terminal Work Item result, then close according to scope. A task coordinator with a current parent Work Item calls `work.aggregation.get` before each `work.aggregation.decide` or `work.aggregation.retry`, passes the current `aggregateRevision` as `expectedAggregateRevision`; `work.aggregation.correct` follows the same read-before-mutate rule and additionally requires `expectedChildResultRevision`, and reads again after each mutation. `work.aggregation.list` may use bounded `depth`, `fields`, `state`, `decision`, and cursor filters for root-authorized read projection; read visibility does not grant mutation authority over descendants. When `work.aggregation.retry` returns a replacement Work Item, call `turn.options` for its target and dispatch a new `turn.run` or `turn.enqueue` with the replacement ID in both `workItemId` and the delegation prompt, using a new Turn idempotency key. If a terminal result changes, use `work.result.correct`, re-confirm affected decisions with `work.aggregation.correct`, correct the affected parent result with `work.result.correct`, and re-finalize it with `work.result`, passing the parent current `expectedResultRevision` and (when applicable) `expectedAggregateRevision`; child result revisions are checked by the correction owner. Do not treat a stale aggregate as a current final result. After every planned direct child or replacement is terminal and decided, it gets the aggregation once more and passes that revision to its own strict parent `work.result`. A root overall coordinator instead calls `work.get` for each top-level child, validates and adopts its terminal result, records the integrated state with `work.history.append`, and submits the self-owned root result only after every descendant and nested aggregation decision is terminal and settled; when it creates a top-level replacement, it must dispatch that replacement through the same `turn.options` and new-Turn sequence.
...
3. Create a Session only with the current actor Session revision as `expectedContainerRevision`, an explicit title, provider, catalog revision, workspace selection, and caller-owned idempotency key.
4. Call `turn.options` for the target Session. Construct a provider-specific Turn tuple only from that result.
5. When the Turn performs a tracked delegation, create or select its active Work Item and pass `workItemId`. Read the target Session's current revision and pass it as `expectedContainerRevision`. Choose `turn.run` for immediate admission or `turn.enqueue` for the persistent FIFO. Never convert one to the other after a busy or queue error.
6. Retain the returned `executionId`. Poll with `turn.get` or `turn.list` when a deferred execution must be followed.
7. If an unresolved interaction appears, inspect its `decisionClass` and `revision`. Do not call `interaction.respond` for `user_only` or `deny_or_cancel`; provider approval and elicitation are currently `user_only` and require the trusted GUI.
...
- Use the lifecycle operations in the operation reference for contract revision, reassignment, parent move, clone, successor reopen, archive, restore, and deletion. Check the current grant and resource revision before mutation. Moving a child adopts its membership, not its result; the new parent must make a separate result decision. A same-root move marks affected finalized parent and ancestor aggregations stale; reconfirm the decisions and corrected results before re-finalizing. A cross-root Work Item move requires the destination target revision and an independently authorized destination proof, preserves the old result, execution, and history, and rejects items with aggregation descendants. A root Session transfer moves the root-owned Work Item and its ownership-scoped resources under the transfer manifest. Preserve the old branch when creating a successor.
- Treat each execution's public `revision` as the cancel precondition. Pass it as `expectedRevision` to `turn.cancel`, and refresh the execution before retrying a changed cancellation request.
