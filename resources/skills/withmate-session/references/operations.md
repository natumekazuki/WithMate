...
The CLI and MCP expose the same 56 operations:
...
- Work Item: `work.create`, `work.list`, `work.get`, `work.revise`, `work.reassign`, `work.move`, `work.clone`, `work.reopen`, `work.archive`, `work.restore`, `work.delete`, `work.history.append`, `work.history.list`, `work.transition`, `work.result`, `work.cancel`, `work.aggregation.get`, `work.aggregation.list`, `work.aggregation.decide`, `work.aggregation.retry`
...
`work.move` records departure from the old parent and adoption by the new parent atomically. If an old decision exists, its supersede is recorded in the same transaction. Adoption changes membership only: the new parent must explicitly assess and decide the result. Moves requiring correction of a finalized parent result or stale propagation to ancestors return a conflict until Slice 5 is connected. A standalone Work Item move across roots is not connected in this slice and is not an available capability. Generic result/decision correction and flatten are not available in this slice.
