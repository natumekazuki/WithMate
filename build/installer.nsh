...
!define WITHMATE_SESSION_ALIAS "${WITHMATE_WINDOWS_APPS_DIR}\withmate-session.cmd"
...
!macro customInstall
  ...
  FileClose $0
  FileOpen $0 "${WITHMATE_SESSION_ALIAS}" w
  FileWrite $0 "@echo off$\r$\n"
  FileWrite $0 "setlocal$\r$\n"
  FileWrite $0 "set ELECTRON_RUN_AS_NODE=1$\r$\n"
  FileWrite $0 '"$INSTDIR\${APP_EXECUTABLE_FILENAME}" "$INSTDIR\resources\resources\cli\withmate-session\withmate-session.mjs" %*$\r$\n'
  FileWrite $0 "exit /b %ERRORLEVEL%$\r$\n"
  FileClose $0
