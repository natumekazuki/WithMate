...
describe("GLOSSARY-CHECKOUT-AUTHORITY renderer projection", () => {
  // @test-value v1
  // kind = "invariant"
  // claim = "Glossary read中にbinding generationが変わった場合は旧scope responseを破棄しcurrent scopeを再読込する"
  // oracle = { type = "contract", ref = "docs/adr/021-agent-runtime-binding-authority-boundary.md" }
  // failure_mode = "失効bindingで読んだGlossary projectionが新generationのSessionへ表示される"
  // scope = "glossary-session-projection-generation"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "contract"
  // claim = "Glossary fileがinvalid化したらlast valid entriesを返さず、Session searchもapplication serviceと同じvalidation errorを返す"
  // oracle = { type = "contract", ref = "docs/adr/022-repository-glossary-boundary.md" }
  // failure_mode = "破損後もstale valid entriesを表示するか、renderer検索だけvalidationを迂回する"
  // scope = "glossary-session-invalid-projection"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "invariant"
  // claim = "並行するGlossary watch deliveryは新しいeventを優先し、遅れて完了した旧stateをSessionへ通知しない"
  // oracle = { type = "contract", ref = "docs/adr/022-repository-glossary-boundary.md" }
  // failure_mode = "out-of-order responseがcurrent projectionを古いinvalid stateで上書きする"
  // scope = "glossary-session-watch-ordering"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "invariant"
  // claim = "並行scope re-armはstale attemptを破棄し、dispose時に作成済みwatch handleをすべて閉じる"
  // oracle = { type = "contract", ref = "docs/adr/022-repository-glossary-boundary.md" }
  // failure_mode = "古いcheckout watcherが残って誤通知するか、dispose後もresourceを保持する"
  // scope = "glossary-session-watch-lifecycle"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "invariant"
  // claim = "Session workspaceが同じでもbinding generation変更時はwatchを張り直してcurrent scopeを再読込する"
  // oracle = { type = "contract", ref = "docs/adr/021-agent-runtime-binding-authority-boundary.md" }
  // failure_mode = "generation失効後も旧watchとscope revisionを使い続け、current authorityを反映しない"
  // scope = "glossary-session-binding-rearm"
  // lifecycle = "permanent"
  // @end-test-value
