...
import { MateTalkChatWindow } from "../../src/chat/MateTalkChatWindow.js";
...
function renderPanel(options?: {
  ...
  return renderToStaticMarkup(
    React.createElement(MateTalkChatWindow, {
      mateName: "ユニバーサル",
...
test("MateTalkChatWindow は Session 共通レイアウトで通常状態をレンダリングする", () => {
...
test("MateTalkChatWindow は空白入力で送信を抑制する", () => {
...
test("MateTalkChatWindow は sending 中に共通の pending 表示と disabled を反映する", () => {
...
test("MateTalkChatWindow は Session と同じヘッダー格納ハンドルを使う", () => {
...
test("MateTalkChatWindow は user/mate メッセージを共通 message row でレンダリングする", () => {
...
test("MateTalkChatWindow は feedback を共通 composer feedback で表示する", () => {
...
test("MateTalkChatWindow は送信中に共通メッセージリストへ pending row を追加する", () => {
  const html = renderPanel({
    sending: true,
    input: "おはよう",
