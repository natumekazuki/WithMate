...
test("MainBroadcastFacade は payload を組み立てて WindowBroadcastService へ委譲する", () => {
  ...
  const facade = new MainBroadcastFacade({
    getWindowBroadcastService: () => ({
      ...
      broadcastModelCatalog(payload: ModelCatalogSnapshot) { calls.push(`catalog:${payload.revision}`); },
      broadcastAppSettings(_payload: AppSettings) { calls.push("settings"); },
      broadcastPromptTemplates(payload: unknown[]) { calls.push(`templates:${payload.length}`); },
      broadcastOpenSessionWindowIds(payload: string[]) { calls.push(`windows:${payload.length}`); },
      broadcastOpenCompanionReviewWindowIds(payload: string[]) { calls.push(`reviews:${payload.length}`); },
    }) as never,
