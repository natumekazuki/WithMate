...
async function createStorageFixture(): Promise<{ directory: string; dbPath: string; storage: SessionExecutionStorageV6; db: DatabaseSync }> {
  ...
  db.prepare("INSERT INTO sessions_v6 (id,title,state,provider_id,catalog_revision,model_id,approval_mode,workspace_path,created_at,updated_at,last_active_at) VALUES (?,?,'active','codex',1,'gpt-5','on-request',?,?,?,?)")
    .run("session-1", "Session 1", process.cwd(), CREATED_AT, CREATED_AT, CREATED_AT);
...
// oracle = { type = "contract", ref = "02-work-item-lifecycle source identity" }
...
test("同期running作成はrevision/planned/actual sourceを同時保存する", async () => {
  ...
  try {
    ...
    const row = fixture.db.prepare("SELECT work_item_revision,actual_source_json FROM work_item_execution_associations_v6 WHERE execution_id='exec-1'").get() as { work_item_revision: number; actual_source_json: string };
...
test("queued admissionのactual snapshotは再open後も保持しtamperを拒否する", async () => {
  ...
  assert.equal(admitted?.actualStartSourceIdentity?.kind, "resolved");
  fixture.storage.close();
  fixture.db.close();
  const reopened = new SessionExecutionStorageV6(fixture.dbPath);
  try {
    assert.equal(reopened.get(queued.id)?.actualStartSourceIdentity?.kind, "resolved");
    reopened.close();
