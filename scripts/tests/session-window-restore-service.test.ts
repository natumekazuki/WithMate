...
describe("SessionWindowRestoreService", () => {
  // @test-value v1
  // kind = "invariant"
  // claim = "保存済みSession集合をruntime再生成後に各一Windowで復元し、以後のcloseを次回snapshotへ反映する"
  // oracle = { type = "contract", ref = "docs/features/session-window-restore.md" }
  // failure_mode = "復元対象が重複openする、欠落する、または復元後のcloseが永続状態へ反映されない"
  // scope = "session-window-restore-roundtrip"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "invariant"
  // claim = "missing・load failure・open failureを対象ごとにskipし他Sessionの復元を継続する"
  // oracle = { type = "contract", ref = "docs/features/session-window-restore.md" }
  // failure_mode = "一対象の失敗で復元batch全体が停止し健全なSession Windowも開かない"
  // scope = "session-window-restore-per-target-failure"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "invariant"
  // claim = "snapshot保存はopen順を保ってSession IDをdeduplicateし最大100件へ制限する"
  // oracle = { type = "contract", ref = "docs/features/session-window-restore.md" }
  // failure_mode = "重複Windowまたは無制限IDが永続化され起動時の復元量がboundedでなくなる"
  // scope = "session-window-restore-snapshot-limit"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "regression"
  // claim = "起動時は復元snapshotをreadしてから現在のopen集合を保存し、未消費対象を上書きしない"
  // oracle = { type = "contract", ref = "docs/features/session-window-restore.md" }
  // failure_mode = "startup直後の空集合保存が復元対象を消しWindowを再生成できない"
  // scope = "session-window-restore-startup-order"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "invariant"
  // claim = "snapshot保存失敗をqueue内でsettleし、復元集合readと後続保存を継続する"
  // oracle = { type = "contract", ref = "docs/features/session-window-restore.md" }
  // failure_mode = "一度のwrite failureで永続化queueが恒久rejectし復元と次回保存が停止する"
  // scope = "session-window-restore-storage-queue-recovery"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "invariant"
  // claim = "復元commit後の通知失敗は復元結果と未復元集合の確定へ波及させない"
  // oracle = { type = "contract", ref = "docs/features/session-window-restore.md" }
  // failure_mode = "Window復元済みなのに通知errorで全体失敗となり同じSessionを再復元する"
  // scope = "session-window-restore-post-commit-notification"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "invariant"
  // claim = "すでにopenのSessionはsnapshotから消費し、実際に新規openしたSessionだけを復元結果へ含める"
  // oracle = { type = "contract", ref = "docs/features/session-window-restore.md" }
  // failure_mode = "既存Windowを重複生成する、または復元していないSessionを成功件数へ含める"
  // scope = "session-window-restore-open-deduplication"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "regression"
  // claim = "手動open中の同一Sessionへ合流し、load失敗時は再試行集合へ残しつつ後続対象を復元する"
  // oracle = { type = "contract", ref = "docs/features/session-window-restore.md" }
  // failure_mode = "同一Sessionを重複生成する、失敗対象を消費する、または後続復元を停止する"
  // scope = "session-window-restore-concurrent-open-failure"
  // lifecycle = "permanent"
  // @end-test-value
  ...
  // @test-value v1
  // kind = "regression"
  // claim = "手動open中の同一Sessionへ合流し、load成功時はWindowを重複生成せず復元成功に分類する"
  // oracle = { type = "contract", ref = "docs/features/session-window-restore.md" }
  // failure_mode = "競合したmanual/restore openが二Windowを作る、または成功Sessionを未復元として残す"
  // scope = "session-window-restore-concurrent-open-success"
  // lifecycle = "permanent"
  // @end-test-value
