...
test("buildMateInstructionContent は priority ガードを最初に出力する", () => {
  ...
  assert.equal(content.indexOf("## Priority") < content.indexOf("### Identity"), true);
  assert.ok(content.includes("coding correctness"));
  assert.ok(content.includes("safety / security"));
});
