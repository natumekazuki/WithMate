...
import type { SessionQueuedTurn } from "../../src/session-turn-execution.js";
...
// @test-value v2
// kind = "invariant"
// claim = "通常SessionのMain列はConversationMessageColumn経由でもFIFO待機Turnとprojection barrierを保持する"
// oracle = { type = "contract", ref = "src/session-queued-turn-projection.ts" }
// fault = "共有chat shellへの統合時にraw Session再投影がqueue表示を落とす"
// observable = "column propsのmessages・messageKeys・turnExecutions"
// observation_boundary = "component-behavior"
// scope = "conversation-message-column"
// lifecycle = "permanent"
// @end-test-value
test("conversation column はMain列のqueued Turn projectionを維持する", async () => {
  ...
  try {
    await act(async () => {
      ...
      root.render(React.createElement(function Probe() {
        latest = useConversationMessageColumn({
          session: {
            id: "main",
            messages: [{ role: "user", text: "先行の依頼" }],
            runState: "idle",
          },
          baseProps: { ...createBaseProps("main"), turnExecutions: [queuedTurn] },
          enabled: true,
    ...
    assert.deepEqual(latest?.messages.map((message) => message.text), ["先行の依頼", "待機中の依頼"]);
    assert.equal(latest?.messageKeys?.[1], "turn-execution-execution-queued");
    assert.equal(latest?.turnExecutions?.[1]?.executionId, "execution-queued");
  } finally {
