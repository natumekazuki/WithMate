...
it("character-authoring runtime metadataだけをatomicにclearしてmessageとrunning stateを変更しない", async () => {
  ...
  try {
    ...
    const initial = storage.insertSession({
      ...createSessionInput("authoring-runtime-clear", [{ role: "assistant", text: "existing" }], "character-authoring"),
...
it("character-authoringのrunning turn開始は最新snapshotと表示metadataをatomicに保存して失敗時は戻す", async () => {
  ...
  try {
    ...
    insertCharacter(dbPath, oldSnapshot);
    const initial = storage.insertSession({
      ...createSessionInput("running-authoring", [{ role: "assistant", text: "existing" }], "character-authoring"),
      character: oldSnapshot.name,
      characterIconPath: oldSnapshot.iconFilePath,
      characterThemeColors: oldSnapshot.theme,
      characterRuntimeSnapshot: oldSnapshot,
...
it("character-authoringのsnapshotとthreadをuser messageとatomicにclearして失敗時は戻す", async () => {
  ...
  try {
    ...
    insertCharacter(dbPath, oldSnapshot);
    const initial = storage.insertSession({
      ...createSessionInput("running-authoring-clear", [{ role: "assistant", text: "existing" }], "character-authoring"),
      character: oldSnapshot.name,
      characterIconPath: oldSnapshot.iconFilePath,
      characterThemeColors: oldSnapshot.theme,
      characterRuntimeSnapshot: oldSnapshot,
...
it("snapshot clearを含むrunning turn開始のsequence conflictはsnapshotとthreadを変更しない", async () => {
  ...
  try {
    ...
    insertCharacter(dbPath, oldSnapshot);
    const initial = storage.insertSession({
      ...createSessionInput("running-authoring-clear-conflict", [{ role: "assistant", text: "existing" }], "character-authoring"),
      characterRuntimeSnapshot: oldSnapshot,
      threadId: "thread-old",
    });
...
it("snapshot入力のundefinedは既存値を保持し、通常Sessionのvalueまたはnull更新を拒否する", async () => {
  ...
  try {
    ...
    const authoring = storage.insertSession({
      ...createSessionInput("running-authoring-undefined", [{ role: "assistant", text: "existing" }], "character-authoring"),
      characterRuntimeSnapshot: oldSnapshot,
      threadId: "thread-old",
    });
    storage.appendRunningTurnStart({
...
it("snapshot clearを含むrunning turn開始のmessage write件数はshortとlongの履歴量に比例しない", async () => {
  ...
  for (const historyMessageCount of [4, 284]) {
    ...
    try {
      ...
      ...createSessionInput("running-count", messages, "character-authoring"),
        characterRuntimeSnapshot: oldSnapshot,
        threadId: "thread-old",
      });
