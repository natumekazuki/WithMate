...
// @test-value v2
// kind = "invariant"
// claim = "専用composerを使うConcurrent chatではMain / Auxiliaryのtarget操作をActionDockとshortcutから抑止する"
// oracle = { type = "contract", ref = "schedule draft keeps Main-only input authority" }
// fault = "schedule draftの保存操作中に通常会話のtarget切り替えが表示または有効になり、Auxiliary送信へ混線する"
// observable = "target dockの不在とConcurrent chat shellの継続表示"
// observation_boundary = "component-behavior"
// scope = "concurrent-chat dedicated composer target boundary"
// lifecycle = "permanent"
// distinction = "通常のConcurrent chat shellでtarget操作を維持する検証とは分け、専用composer時の入力対象境界を確認する"
// @end-test-value
test("ChatWindow は専用composer中のConcurrent chat target操作を抑止する", () => {
  const props = createChatWindowProps();
  ...
  assert.match(html, /session-concurrent-chat-columns/);
  assert.doesNotMatch(html, /操作対象チャット/);
  assert.doesNotMatch(html, /concurrent-chat-target-dock/);
});
