...
- `Project Memory` の正本は将来の `project_memory_entries`
...
- target 設計では、Codex は `System Prompt -> Character -> Session Memory -> Project Memory -> User Input` を 1 本の text prompt として組み立てる
- target 設計では、Copilot は `System Prompt Prefix + character.md` を `SessionConfig.systemMessage` に置き、`Session Memory + Project Memory + User Input` を `session.send()` 側へ載せる
