...
Mate Core / Bond Profile / Work Style は provider instruction file へ同期し、毎 turn prompt へ全文合成しない。app 共通 system prompt も挿入しない。
...
論理 prompt では Mate 定義全文 section と app 共通 system section を常設しない。
...
Mate Profile から provider instruction file の managed block へ同期する短い投影。
...
- 固定システム指示はアプリ実装にも app settings にも持たない
- Mate Profile は provider instruction projection の主要入力として扱う
...
- `Settings Window` では provider instruction sync の root / path / write mode を編集できるようにする
