...
baselineにないtargetへの通信には、そのtargetをscopeに含む明示grantを必要とする。cross-rootではresource・budget・expiry・返却先を限定したconsultation grantを使い、権限のないtargetや存在しないtargetはexecution、queue、Coordination Eventを作る前に拒否する。API要求はreplayを含め現在のbindingとactive grantを評価する。自己宛を除くdirect dispatchはWork Itemまたは一時consultation grantを関連付け、自由文だけの追跡不能な依頼にしない。公開requestでは`workItemId`または`consultationGrantId`を指定し、実行結果にもconsultation grantの識別子を保持する。新しいWork Item lifecycle capabilityは既存baseline grantへ自動追加せず、trusted root policyまたはそのceiling内のgrant.createで明示付与されたactorだけが実行できる。許可された同一principalの再送には保存済みcanonical resultを返し、新しいexecutionを作らない。GUI送信はtrusted user invocationとして同じexecution ownerを使うが、このAgent間authorityの対象にはしない。
...
作成はruntime bindingで確定したactor、active grant、canonicalな同一rootのtarget relationで認可する。targetはactor自身ではない同じrootのSessionを明示し、parent Work Itemを指定する場合は同じrootでactor Sessionがtargetとなっているactiveなdelegated Work Itemに限定する。固定の直属parent／sibling／grandchild matrixやRole名だけをlive authorityとして扱わない。root coordinatorが作成するtop-level delegated Work Itemは`parentWorkItemId = null`とし、Root Work Itemをdelegation parentへ指定できない。target Sessionは`pending -> in_progress -> waiting -> in_progress`の進行操作とterminal result報告を行い、creator Sessionはactive Work Itemを取消せる。`work.reassign`はassignmentをhandoffまたは新ID successorとして変更し、`work.move`は旧parentからの離脱・新parentへのadoption・必要な旧decisionのsupersedeを一transactionへ保存する。adoptionは結果の自動採用を意味せず、新parentは別途decisionを作る。同一rootで確定済みparentまたはancestorに影響するmoveは、旧結果を保持してstaleを伝播し、移動先では新たなdecisionを要求する。異なるroot間のWork Item単体moveは、移管先targetのcurrent revisionと両rootのgrantを要求し、集約descendantを持つWork Itemは拒否する。旧result・execution・履歴を保持してroot・creator・targetを原子的に移管する。`work.clone`は契約だけを新IDへ複製し、`work.reopen`は旧Work Itemのresult、decision、所属履歴を保持したsuccessorを作る。`work.result.correct`は旧resultを上書きせず新revisionとsupersede関係を保存し、`work.aggregation.correct`は`revise | withdraw | replace` unionでcurrent decisionを更新する。flattenはroot visibilityに限定したbounded depth、field、state、decision filterとcursorを使う。全mutationはexpected revisionとprincipal Session単位のidempotency keyを要求する。GUI editorは開始時revisionをdraftと共に保持し、外部更新でcurrent projectionが進んだ場合は保存を止める。入力を破棄して最新版を読み込むか、draftの契約fieldが最新版と一致する場合に限って進捗入力を明示的に引き継ぐ操作でbase revisionを更新する。自動mergeや最新revisionへの暗黙の付け替えは行わない。API入口でcurrent runtime bindingとactive grantを評価した後、canonical replayは保存先Sessionの現在のrelationの再解決より先に返し、recordは24時間後にcleanupする。
...
Work Item aggregationはactiveなdelegated parentと直属子を基本対象とし、`work.aggregation.list`はroot coordinatorの権限とboundedな`depth`を指定した場合だけ子孫のread projectionを返す。Root Work Itemに対する`work.aggregation.get | list | decide | retry | correct`は`WORK_ITEM_AGGREGATION_PARENT_INVALID`で拒否し、Root Work Itemのresultはaggregate revisionを受け取らない。親target Sessionはterminalな直属子へ`accepted | excluded | retry_requested`のimmutable decisionを作成する。`excluded`は理由を必須とし、`canceled`は採用できない。retryは明示された新しいbindingでreplacementを作成し、decision、replacement、idempotency resultを同一transactionへ保存する。元Work Itemのstateとresultは変更しない。

`work.aggregation.get | list`はaggregate revisionと件数、および指定されたfield projectionだけをbounded queryで返す。full resultは`work.get`で取得する。cursorはparent、actor、visibility、depth、field、state、decision filterへ束縛する。`work.result.correct`は旧resultを更新せず、expected result revisionと理由を要求して新しいrevisionを保存し、旧resultを受け入れたdecisionと親aggregateをstaleへ伝播する。`work.aggregation.correct`は`revise | withdraw | replace`のstrict unionで旧decisionを保持したままcurrent projectionを一件だけ更新する。staleなaggregateはcurrent finalとして新たに採用できない。直属子を持つ親の`work.result`はcurrent aggregate revision、active decision、child result revisionを同一transactionで検証し、親resultとfinalized eventを保存する。親terminal確定後は直属子追加とdecision mutationを拒否する。
`depth`は0〜8、`fields`は`summary | decision | provenance`の重複なし最大3項目、list limitは既存aggregation上限、公開responseは8 MiB上限としてruntime catalogへ反映する。
