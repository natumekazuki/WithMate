...
React/Vite の separate entry renderer を Electron の実 `BrowserWindow` へ接続し、
...
- window の上位責務と構成判断は `docs/design/window-architecture.md` を正本とする
- current 実装では runtime detail、`BrowserWindow` 管理、preload 境界を主題にする
...
- renderer は `window.withmate` を唯一の実行境界として扱う
...
- Session / Character / Diff の renderer をどこまで疎結合に保つか
