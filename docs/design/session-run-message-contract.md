...
- initiating user Message と Run の作成は、片方だけが残らないよう同じ application transaction で受理する。
...
`phase=completed` かつ `durabilityState=failed` は許可する。live UI / CLI の実行完了表示は SQLite commit を待たないが、未永続の状態を復旧可能とは表示しない。永続化失敗は Provider 実行失敗と別の診断・再試行対象にする。
...
2. user Message、`queued` Run、Session の `executionState=running` と `activeRunId` / `latestRunId` を同じ domain transition で受理する。
3. Run execution snapshot を確定する。
4. Provider process と ProviderBinding を準備し、`starting` へ遷移する。
5. Provider request を送り、外部 ID を相関させる。
...
10. 確定した Run、Message、RunEvent の永続化を Persistence Worker / actor へ依頼し、durability state を独立して更新する。
...
- 同じ active Run に関連付け、Provider の steer / prompt request へ変換する。
