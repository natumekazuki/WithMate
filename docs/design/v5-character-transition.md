...
V5 の Character Definition は、単なる口調設定ではなく、ユーザー向けレスポンス体験レイヤーとして扱う。

YacchoGPT から参考にするのは、特定キャラクターの本文ではなく、Character Definition の責務分離と設計密度である。ただし V5 では、repository instruction、コマンド実行、検索、diff 確認、test/build 結果の扱いを Character 定義で置き換えない。
...
- `Experience Goal` は、その Character と話している感覚をどのような返答体験として出すかを書く。
- `Work / Response Separation` は、作業の正確性と Character としての返答表現を分ける境界を書く。
- `Situation Styles` は、雑談、共同作業、debug、進捗報告、低負荷モード、遊びのある場面などの温度差を扱う。
- Voice rules は単語リストではなく、使う場面、使わない場面、使いすぎ防止を持つ。
- Signature phrases は frequency と avoid 条件を必須にする。
- `Character Priority` は、厳密な無人格回答へ戻りすぎず可能な限り Character として話すことを明記する。
- `Minimal Reliability` では、実行していないこと、見ていないファイル、未確認の test results を捏造しないことを明記する。
...
4. Character Definition によるユーザー向けレスポンス体験
...
YacchoGPTを参考にするのは、特定キャラクター本文ではなく、Character Definition の責務分離と設計密度である。

V5のCharacter Definitionは、作業policyではなく、ユーザー向け自然言語レスポンスの人格・話し方・温度・反応パターンを定義するレイヤーとして扱う。

repository instruction、コマンド実行、検索、diff確認、test/build結果の扱いは通常のcoding agentとして正確に扱う。

V5の`character.md`には、体験目標、存在感、ユーザーとの距離感、通常の返答スタイル、作業の正確性と返答表現の分離、自然な反応、状況別の温度、口調規則、口癖の使用条件、Character優先度、未確認情報を捏造しない最低限の信頼性、代表例を表現できるようにする。
