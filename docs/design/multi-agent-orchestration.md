...
`waitChild` / `waitAny` / `waitAll`はread-onlyのreadiness待機とし、readyになったChildHandleと状態だけを返す。ChildResult本文は返さず、Delivery、RunEvent、IdempotencyRecordを変更しない。初回回収を記録して親Runのtool resultへ結果を渡す操作は、単一handleを受ける`collectChildResult`だけとする。

`collectChildResult`はcollecting parent Run IDとidempotency keyを受け、応答送信直前に切断しても単一Delivery参照から同じ結果envelopeを再送できるようにする。回収済みでも同じidempotency keyには同じ結果を返す。結果本文はchild Sessionを基準とするため、child Sessionが参照可能な間は別の親RunからもSession ID / Delegation IDを使って再取得できる。初回回収記録は再取得禁止を意味せず、後続の回収で上書きしない。
...
waitChild(handle, timeout?) -> ReadyChildHandle | StillRunning
waitAny(handles, timeout?) -> ReadyChildHandle[] | StillRunning
waitAll(handles, timeout?) -> ReadyChildHandle[] | PartialReadiness
...
同時実行上限は `orchestrationRootSessionId` 単位で、その配下の non-terminal child Run 件数へ適用する。これとは別に、Application Serviceは通常Run、Auxiliary、child Runを合算するapp全体hard capと、Provider runtime / local process種別ごとのhard capを持つ。初期defaultはroot child 2、app全体4、同一Provider runtime 4、low-resource profileはroot child 1、app全体2、同一Provider runtime 2とし、user設定でhard capを超えられない。

A が B と C を直接起動した場合も、A が B、B が C を起動した場合も同じroot上限を共有する。初回 `startChild` だけでなく、`clarification_required` 後の再開、retry、既存 child Session での後続 Run 開始も同じadmissionを通る。実効可否は`min(root残枠, app全体残枠, 対象Provider / process残枠)`で決め、いずれか0なら開始しない。Sessionの`executionState=running`はnon-terminal Runから導出し、capacity集計には`runs`、`run_attempts`、`provider_bindings`、`session_relations`を使う。
...
並行するRun開始で上限を超えないよう、root、app全体、Provider runtimeの各使用数確認と、initiating user Messageへの参照、`queued` Run、Attempt、Delegation workflowの更新を1つのSQLite write transactionとして直列化する。通常RunとAuxiliaryも同じglobal / Provider admissionを通す。新しいinstructionまたはclarificationではinitiating user Messageも作成し、exact retryでは既存Messageを参照して新規作成しない。初回`startChild`では子Session / relation / Delegationも同じtransactionで作成し、既存child Sessionの再開やretryでは再作成しない。受理したRunはProvider起動前から各slotを消費する。
...
parent running
  ...
  -> child terminal
  -> waitChild が ready handle を返す
  -> collectChildResult が結果を durable tool result として返す
  -> parent running
```
...
start child B
親側の処理を継続
waitAny / waitAllでready handleを取得
ready handleごとにcollectChildResultで結果を回収
親側の処理を継続
```
...
完了結果を親へ知らせるために、子から親へ途中 Message を送信したり、親 Run を完了まで無期限に保持したりしない。同期的に使いたい Agent は`waitChild` / `waitAny` / `waitAll`でready handleを待ち、続けてhandleごとに`collect`する。バックグラウンド継続を選んだAgentは後続turnで一覧・参照・`collect`を明示的に行う。
...
子 Agent が instruction を曖昧と判断した場合、作業を推測で続けず、`clarification_required`、不足している情報、確認事項を ChildResult として即座に確定する。`wait`でreadyを確認し、本文の配送は`collect` contractを使う。評価登録の成否に依存しない。
...
4. `start`、`collect`、`cancel`の再送はidempotencyを保ち、同じkeyには同じChildHandleまたは結果envelopeを返す。`child.start`のkeyはparent Sessionへscopeし、child delete後もparent存続中は再利用させない。read-onlyな`wait`は再実行時点のready handle snapshotを返し、結果本文を渡さず回収も記録しない。
...
GitHub #283の方針に従い、runtime hostのApplication boundaryから同期的にSQLiteを操作しない。親子relation、Delegation、RunEvent、terminal outcome、配送状態、評価recordのwriteはruntime hostが所有するPersistence Worker / actorを経由する。Electron Main process、Renderer、CLIはWorker clientへ直接到達しない。process ownershipはADR 013を正本とする。

live UIの完了表示をSQLite commit待ちにしない一方、親turnをまたぐ未回収結果は再起動後にも発見できる必要がある。このため、実行状態とruntime hostのlive persistence stateを分離し、バックグラウンド結果を「復旧可能」と扱うのは必要なdomain recordがdurable commit済みの場合だけとする。
...
- 自動retention期間とProvider側会話削除の連動はpersistence / privacy設計で後続定義する。初期版のuser明示deleteは`local_only`で、対象Session subtreeと関連local dataを物理削除するがProvider側Thread / Sessionの削除は保証しない。親scopeの冪等key tombstoneだけは親Session存続中に残す。
...
- `waitChild` / `waitAny` / `waitAll`がready handleだけを返し、Delivery / IdempotencyRecordを変更せず、結果ごとの明示`collect`だけがChildResultを親tool resultへ渡して初回回収を記録すること
...
- 複数root、通常Run、Auxiliary、child Runの合計がapp全体 / Provider runtime hard capを超えず、low-resource profileでも同じadmissionを通ること
- closed Delegationの明示retryだけが新しいRunと同じtransactionで`closed -> active`へ進み、repairや拒否されたretryでは再開しないこと
- `child.start -> child terminal -> child subtree delete -> 同一key再送`で新しいchildを作らず`idempotency_expired`を返すこと
