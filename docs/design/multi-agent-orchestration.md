...
`waitChild` / `waitAny` / `waitAll`はread-onlyの待機操作とし、Delivery、RunEvent、IdempotencyRecordを変更しない。複数結果を返すwaitのresponse envelopeを永続化せず、再実行時はその時点のsnapshotを返す。初回回収を記録して親RunのRunEvent / tool resultへ関連付ける操作は、単一handleを受ける`collectChildResult`だけとする。
...
waitChild(handle, timeout?) -> ChildResult | StillRunning
waitAny(handles, timeout?) -> ChildResult | StillRunning
waitAll(handles, timeout?) -> ChildResult[] | PartialResults
...
4. `start`、`collect`、`cancel`の再送はidempotencyを保ち、同じkeyには同じChildHandleまたは結果envelopeを返す。read-onlyな`wait`は再実行時点のsnapshotを返し、結果回収を記録しない。
...
- `waitAny` / `waitAll`の再実行がDelivery / IdempotencyRecordを変更せず、結果ごとの明示`collect`だけが初回回収を記録すること
