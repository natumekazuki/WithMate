...
- session CRUD と upsert は `SessionPersistenceService` が担う
- turn 実行は `SessionRuntimeService`、Session Window lifecycle は `SessionWindowBridge` が担う
...
- `SessionPersistenceService` を通して `createSession` / `updateSession` / `deleteSession` / `upsertSession` を扱う
