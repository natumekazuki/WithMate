...
`Session Memory` の裏実行は、通常の user turn と同列の監査ログに混ぜない。  
ただし「いつ・どの設定で・何が保存されたか」は後から追えるようにする。

### Phase

memory extraction 用には次の phase を使う。
...
background memory extraction でも `audit_logs` table 自体は共用する。  
ただし内容は次のように使い分ける。
...
current design では memory extraction も「実際に provider へ投げた background task」であり、監査粒度は通常 turn と近い。  
そのため first implementation では table を増やさず、`phase` と payload の区別で扱う。
