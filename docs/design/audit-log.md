...
approval は UI 上では `Auto Run / Provider Controlled / Safety Focused` の provider-neutral wording で表示する。
そのため、保存値が legacy/native でも overlay 表示時には canonical mode へ normalize したうえで同じ表示labelに揃える。
