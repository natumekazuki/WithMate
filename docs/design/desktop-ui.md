...
- Sessionのスケジュール表示はTemplateと同様に中央surfaceだけを差し替え、左右の補助paneとsplitterを維持する。一覧ではActionDockだけを隠し、作成または編集へ入ったときだけActionDockをスケジュール入力として表示する。Cron式の直下には次の5回の実行予定をOSローカル時刻で表示し、入力欄のラベル、編集画面の見出し、入力前の補足文など、操作から推測できる文言は常設しない
