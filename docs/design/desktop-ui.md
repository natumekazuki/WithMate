...
Session WindowはMain左と選択中Auxiliary右を同じchat shellで表示できる。Auxiliaryは複数保持し、Headerの`New Auxiliary`は既存会話を閉じず末尾へ追加する。Auxiliary中央の左右矢印と表示名一覧はstable Session IDで選択し、一覧行はCharacter iconと非AIの会話previewだけを表示する。折りたたみ時はAuxiliary面と内部境界を隠す。表示幅の開閉とActionDockの送信対象は独立し、開閉では対象を変更しない。
...
- scheduleの戻る、追加、編集、pause、resume、run now、delete、保存は意味が一意なicon-only controlとし、visible textを置かない場合も`aria-label`と`title`を持たせる。状態、Cron式、次回予定、最終実行時刻、実行結果（成功／エラー）は文字で表示する。Execution IDは内部処理にのみ使い、一覧へ表示しない。time zoneは保存時のOSローカルzoneを内部へ記録し、入力項目や一覧項目として表示しない
- Sessionのスケジュール表示はTemplateと同様に中央surfaceだけを差し替え、左右の補助pane、splitter、ActionDockを維持する。作成・編集時だけActionDockをスケジュール用入力へ切り替える。空一覧では見出しと作成操作だけを表示する。Cron式の直下には次の5回の実行予定をOSローカル時刻で表示する。入力欄のラベル、編集画面の見出しなど、操作から推測できる文言は常設しない
