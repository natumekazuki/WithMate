...
`dispatching`または`ambiguous` Dispatchは未送信として処理せず、`preDispatchResolution=not_applicable`でも拒否する。`ambiguous`はProvider照合による`accepted`への知識補正をRun / Attemptの終端より先に確定する。exact replay fingerprintはcanonical JSONから生成するためobject key順には依存せず、意味が異なる再実行は拒否する。caller証明を含む判断理由は`docs/adr/003-pre-dispatch-run-terminal-resolution.md`を正本とする。
