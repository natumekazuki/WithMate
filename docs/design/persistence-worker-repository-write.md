...
completed recordの保持期間はWorker所有の30日とする。期限はcaller入力にせずcommit時刻から計算する。期限到達済みrecordを観測したwrite transactionはresponse referenceとenvelopeを消去して`expired`へ進め、Session明示削除までkey tombstoneを保持する。response envelopeは16 KiB以下とし、Message本文、child結果、payload、raw Provider responseを保存しない。
...
`repository.session.create`はactive Sessionとself-scopeのcompleted IdempotencyRecordを同じtransactionで作成する。ProviderBindingは作成しない。
...
archive / closeはnon-terminal Runがない場合だけ許可する。unarchiveはworkspaceとSession Providerを再検証し、存在するopen ProviderBindingが同じProviderの`active` Bindingである場合、またはBinding未作成で次Run時に作成可能な場合だけ許可する。未収束の`creating` BindingはProvider照合・修復前に再開根拠として使わない。`closed`からの再開、same-state update、expected state不一致を拒否する。`updated_at`は更新するが、metadata transitionだけで`last_activity_at`を進めない。
