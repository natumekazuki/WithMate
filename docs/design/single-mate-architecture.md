...
- 3.x の character catalog / Character Editor / Character Update / Character Memory 詳細は legacy archive としてのみ扱う

## Core Decisions

- WithMate 4.0.0 は完全 SingleMate とする
- 既存 character catalog から Mate への migration は行わない
- Character Editor、character catalog API、Character Update workspace、Character Memory / Character Reflection runtime は 4.0.0 runtime から削除する
- `character.html` entry、renderer editor app、main IPC、preload API、window lifecycle、provider reflection adapter hook は残さない
...
- 旧 `sessionKind = "character-update"` の persisted value は migration/history 互換の legacy marker としてのみ扱い、新規導線は作らない
...
4.0.0 方針では、通常の coding session とは別に Mate を対話的に育てる画面として、メイトークを持つ。
V5 preview では legacy MateTalk runtime / window / `mate-talk` chat mode を current runtime から外し、Character Chat が必要になった場合は V5 Character catalog から選択した Character を使う別 Issue として再設計する。
...
- UI 上は対象 Mate の directory を扱うが、provider session は app data 配下のメイトーク用 projection workspace で起動する
...
- approval / sandbox は通常 Session と同じ権限選択 UI を使い、選択値を provider の background 実行へ渡す
- Web 検索、ファイル参照、追加ディレクトリ参照は通常 Session と同じ権限モデルで扱う
- Codex sandbox selector は Codex provider の時だけ表示し、Copilot など sandbox を解釈しない provider では表示しない
- projection workspace への隔離は補助防御であり、WithMate 管理領域や protected directory は通常 Session と同じ guard で守る
...
- skill / agent など MateTalk の実行 payload に接続していない作業用操作は非表示にする
- header collapse、message follow、right pane shell、keyboard submit などの基本挙動は Session と同じにする
- right pane は構造として残すが、メイトーク専用の説明文や空 state copy を常設しない
...
- window header / collapsed title handle
- message list
- assistant / user bubble
- pending / streaming 表示
- composer layout
- Model / Depth selection
- Send / Cancel / keyboard submit
- right pane shell
- splitter / responsive layout

mode ごとに切り替えるもの:

- 送信先 service
- session persistence / history source
- 実行中 state の取得元
- composer toolbar に出す操作
- right pane に表示する内容
- audit / task / command monitor の有無

V4 legacy メイトークでは、通常 Session と同じ composer control のうち、file / folder / image / additional directory / approval / model / depth を表示し、選択値を background provider 実行 payload に渡す。
