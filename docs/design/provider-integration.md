...
- WithMate Session は Provider 種別、外部会話 ID、protocol version、CLI version などの対応情報を保持できるようにする。
- DB schema、event ledger、object storage の具体構造は persistence design で別途決める。
...
- Session / Run / Message / RunEvent の責務と不変条件は `docs/design/session-run-message-contract.md` を正本とする。
...
### 6. Provider 共通の Run phase / activity は WithMate が管理する

Provider は実際の処理状態と Provider 固有 event を所有する。WithMate はそれらを共通 Run phase、activity、pending interaction、terminal outcome へ変換し、CLI / GUI へ提供する。

- phase は `queued`、`starting`、`active`、`canceling`、`finalizing` と terminal phase を表す。
- activity は active Run の `running`、`waiting_approval`、`waiting_input` を表す。
- approval / elicitation の request 本体は activity へ埋め込まず、request ID ごとに管理する。
...
- terminal event 未受信時に永続 Thread を再開し、Provider へどこまで再照会できるか。
