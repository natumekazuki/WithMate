...
- V1〜V4 Memory / Growth と V5 current runtime policy の入口はこの文書とする
- V6 Memory foundationの正本は`docs/design/v6-memory-foundation.md`とする
- V6 DB foundationの正本は`docs/design/v6-database-foundation.md`とする
...
V5 current runtimeでは既存 DB data を削除しない。保存済み `session_memories`、`project_memory_entries`、`character_memory_entries`、`sessions.stream_json`、background `audit_logs` は互換用の既存データとして残す。
V6ではこの保持方針を引き継がず、`docs/design/v6-database-foundation.md`のdestructive reset方針を優先する。
