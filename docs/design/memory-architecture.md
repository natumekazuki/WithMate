...
- 将来的には Memory extraction 用モデルを Settings で切り替えられる余地を残す
- ただし current milestone では UI 設定は必須にしない
- 最初は app 内既定値を持ち、必要になってから設定項目を追加する
...
- ただし `Character Memory` そのものは毎 turn フル投入しない
- 常設する場合も、薄い summary に留める
...
- `Character Memory` は `Project Memory` と同じく常設しないか、ごく薄く扱う
- 主な用途は関係性の補助であり、作業継続の骨格ではない
- Character Stream をやる場合は重要度が上がるが、current milestone では常設しない前提でよい

## Injection Order

current の設計では、実際の prompt 組み込み順は次の考え方を基本にする。

1. app / provider の system 指示
2. `character.md`
3. `Session Memory` summary
4. 必要時に検索した `Project Memory`
5. 必要時に検索した `Character Memory`
6. ユーザー入力
