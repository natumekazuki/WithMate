...
- この文書は `Project Memory` の storage / promotion / retrieval detail を持つ supporting doc として扱う
...
これらは ranking / search layer の follow-up として追加する。
...
current 実装では、昇格判定をかなり保守的にする。

- `decisions`
  - 常に `decision` category として昇格する
- `notes`
...
この slice では、durable knowledge と session 固有 scratchpad を混ぜないことを優先する。

### Why So Conservative

- `goal` は session 固有で変わりやすい
- `openQuestions` は未解決論点であり、durable knowledge とは限らない
- `nextActions` は session 継続用 TODO
- `notes` はノイズが多いため、明示 tag があるものだけを採用する

## Entry Shape v1

昇格後の entry は次の形で保存する。
...
- `detail`
  - durable knowledge の本文
- `keywords`
  - `title` と `detail` から機械的に抽出した軽量 token
- `evidence`
  - current 実装では空配列
...
- retrieval runtime では entry ごとの正規化済み haystack / fingerprint / feature key を module cache で再利用し、feature key の inverted index で user message に一致する candidate だけを scoring する
- `SessionMemory.goal` / `SessionMemory.openQuestions` は candidate の拡張には使わず、user message に一致した candidate の ranking 補助として使う
