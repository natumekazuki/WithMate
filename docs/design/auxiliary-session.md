...
通常closeの保存ACKとapp quitの終了ACKを区別する。通常closeは実行をMain Processで継続できる。app quitは入力を凍結したうえで、選択中でないownerも含めて送信前保存から送信結果確定までを待ち、失敗時に生じた復元pendingを保存してからACKする。凍結中に送信前保存が完了しても新規runを開始しない。全Windowのquit ACK後も、既に閉じたWindowからの送信とMain側の復元保存を待ち、未確定・復元失敗・timeoutのままDBを閉じない。待機timeoutではquitを中止して凍結を解除し、実行は継続する。
