...
- Closed Auxiliary transcript は message column 内で session ごとの境界 label を表示する。
- 短い status label として `Main session paused`。
- `Return to main` は枠 header または ActionDock 側に置く。
...
Audit Log は初期実装ではメイン Session の永続 audit log を開く。
Auxiliary mode から開いた場合も親 Session の log であることが分かるよう、phase label の隣に `Main Session` source tag を表示する。
...
- Auxiliary は既存 parent Session に従属する補助データで、アプリ起動や既存 Session 読み込みの required table ではない。
...
`payload_json` には provider / model / runtime options、AddDirectory snapshot、thread id、composer draft、messages、createdAt / updatedAt / closedAt を保存する。

将来、Auxiliary transcript の検索、message 単位の pagination、artifact の遅延読み込み、監査ログ同等の詳細表示が必要になった場合は、V5 で `auxiliary_messages` などへ正規化する。
