...
`createOrVerifyV6FreshDatabase()`はfresh V6 DB作成と既存valid V6 DB検証だけを行う。fresh作成は一時directory内の`withmate-v6.db`へtransactionでschemaを作り、deep validation後にfinal pathへ既存file非上書きでpublishする。失敗時はtemporary DBとsidecarを削除し、既存invalid V6 DBは破壊的に上書きしない。active runtime DB path selectionにも接続しない。
app ready時のMemory V6 runtime APIは、このhelperでV6 foundation DBをbest-effortで作成または検証する。V6 DBがinvalidな場合はMemory runtimeだけ起動せず、V4 active runtime bootは継続する。
diagnosticsは`withmate-v6.db`を既知fileとして表示するが、V6 foundation DBは`foundation-ready`、`runtimeEligible: false`として扱う。V4 DBと並存しても複数runtime generation warningの対象にしない。active pathとしてV6 foundation DBを渡した場合はschema-validでもcurrent runtime compatibleとは扱わずwarningを返す。
...
6. Memory V6 runtime API用のapp起動時bootstrapを実装する。完了: V4 active runtime selectionを変えずに、V6 foundation DBをbest-effortで作成 / 検証する。
7. V6 release migrationで必要な継続データだけをV6 DBへ自動移行する。
