...
へ分離する。Home は管理ハブ、Session は chat 実行面、Mate Editor は SingleMate 編集面として役割を固定し、`character-update` と `mate-talk` は独立 chat layout ではなく `Session Window` の variant として扱う。Electron 実装時の window lifecycle を明確にする。
...
- `Session Window`
  ...
  - Agent / Companion / メイトークの chat variant
