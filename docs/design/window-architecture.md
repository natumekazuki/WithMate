...
- 作成日: 2026-03-12
- 対象: `Home Window` / `Session Window` / `Character Editor Window` / `Diff Window` / `Settings Window` / `Session Monitor Window` / `Memory Management Window` の責務分離

## Goal

Issue `#2 Homeとセッションは別ウインドウにする` に合わせて、WithMate の UI を
`Home Window` / `Session Window` / `Character Editor Window` / `Diff Window` / `Settings Window` / `Session Monitor Window` / `Memory Management Window`
...
- `Memory Management Window`
  - `Session / Project / Character Memory` の一覧、filter、delete 専用 window
...
- `Session / Project / Character Memory` の一覧
...
- `Session / Settings / Memory Management / Character Editor / Diff / Session Monitor` の新規 window は、cursor がある display の workArea を基準に開く
