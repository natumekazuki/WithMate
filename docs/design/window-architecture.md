...
- `Session Header`
  - session title の変更
  - session title が空の場合の Window title fallback は `Session` とし、内部IDをtitleに表示しない
  - approval mode の変更
  - session 削除
...
- アプリ起動時に 1 つだけ作成する
- 基本は常駐の管理ハブとして扱う
- 全 window を閉じても app process は終了しない。Windows では single instance lock / `second-instance` により、再起動操作で既存 process の Home を再表示・focus する
