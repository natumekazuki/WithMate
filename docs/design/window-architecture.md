...
- 作成日: 2026-03-12
- 対象: `Home Window` / `Session Window` / `Mate Editor Window` / `Diff Window` / `Settings Window` / `Session Monitor Window` / `Memory Management Window` の責務分離

## Goal

Issue `#2 Homeとセッションは別ウインドウにする` に合わせて、WithMate の UI を
`Home Window` / `Session Window` / `Mate Editor Window` / `Diff Window` / `Settings Window` / `Session Monitor Window` / `Memory Management Window`
へ分離する。Home は管理ハブ、Session は chat 実行面、Mate Editor は SingleMate 編集面として役割を固定し、`character-update` と `mate-talk` は独立 chat layout ではなく `Session Window` の mode として扱う。Electron 実装時の window lifecycle を明確にする。
...
- `Mate Editor Window`
  - Mate Profile 編集
  - avatar 設定
  - provider instruction 投影の起点
- `Session Window`
  - coding agent の作業実行
  - `character-update` mode の更新作業実行
  - Agent / Companion / メイトークの chat mode
...
- `Session / Project / Mate Memory` の一覧、filter、delete 専用 window
...
- `Your Mate`
  - SingleMate の要約確認
  - `MateTalk` の別 window 起動
  - Mate Profile / avatar 設定の起点
- `Settings`
  - 独立した `Settings Window` を開く
  - model catalog import / export の起点
  - `Memory Management Window` の起点
- 将来的な session / mate 情報の検索、並び替え

Home に置かないもの:

- `Work Chat`
- `MateTalk` の inline chat
...
ただし chat layout は Session 固有ではなく、Agent / Companion / メイトークが共有する唯一の chat UI とする。
...
- `Session / Project / Mate Memory` の一覧
- global search、domain filter、status / category filter、updatedAt sort
- memory reload
- memory delete

Memory Management Window に置かないもの:

- provider / credential 設定
- model catalog import / export
- DB reset
- chat composer
...
1. ユーザーが `Home Window` の `Your Mate` から `MateTalk` を押す
2. Main Process が `Session Window` を `mode=mate-talk` で新規作成または foreground に出す
3. renderer は Agent / Companion と同じ chat layout を使い、MateTalk 用 adapter だけを差し替える
4. Home には MateTalk の message list / composer / right pane shell を置かない

### Mate Update Session

1. ユーザーが `Mate Editor Window` で `Open Update Workspace` を押す
2. Mate Editor 内で provider picker modal を開く
3. アプリが `sessionKind = "character-update"` の session record を作る
4. `Session Window` を新規作成して、その session を `character-update` mode で開く
...
- `Session / Settings / Memory Management / Mate Editor / Diff / Session Monitor` の新規 window は、cursor がある display の workArea を基準に開く
...
- `Settings Window` と `Session Monitor Window` と `Memory Management Window` の起点を持つ
- split-screen を考慮し、single-column layout が機能する幅まで縮められるようにする

### Mate Editor Window Lifecycle

- Home の Mate Profile / avatar 設定導線から必要時に生成する
- SingleMate の編集 window として 1 window を再利用する
- 保存や削除を行ったら Home の mate summary へ反映する
...
- `mode = "mate-talk"` でも window lifecycle 自体は通常 session と同じ chat window rule に従う
...
### Memory Management Window Lifecycle

- `Home Window` または `Settings Window` から必要時に開く
- 1 window を再利用する
- 閉じても app 全体は継続する
...
- 編集中 mate profile / avatar の入力途中状態
- save / delete の UI 状態
...
mate metadata の正本は file system 上の mate storage とする。
...
- `Mate Editor Window` の生成、再利用、フォーカス
- `Diff Window` の生成
- `sessionId -> BrowserWindow` と mate editor window の対応表管理
- 実行中 session の registry 管理
- launch / resume / mate edit 時の IPC 受け口
...
flowchart LR
    ...
    HW -->|resumeSession| MP
    HW -->|openMateEditor| MP
    HW -->|openMateTalk| MP
    MP -->|create or focus| SW[Session Window]
    MP -->|create or focus| CW[Mate Editor Window]
...
- Mate Editor で更新した内容を開いている Session へどう反映するか
