...
- 作成日: 2026-03-12
- 対象: `Home Window` / `Session Window` / `Character Editor Window` / `Character Update Window` / `Diff Window` / `Settings Window` / `Session Monitor Window` の責務分離

## Goal

Issue `#2 Homeとセッションは別ウインドウにする` に合わせて、WithMate の UI を
`Home Window` / `Session Window` / `Character Editor Window` / `Character Update Window` / `Diff Window` / `Settings Window` / `Session Monitor Window`
へ分離する。Home は管理ハブ、Session は coding agent 実行面、Character Editor はキャラ編集面として役割を固定し、
Electron 実装時の window lifecycle を明確にする。
...
- `Character Update Window`
  - character directory を workspace にした更新支援
  - Memory Extract の確認と copy
  - update 用 Session 起動
- `Session Window`
  - coding agent の作業実行
...
- character ごとの保存ディレクトリを workspace として表示する
- provider ごとの update 用 instruction file の配置先を見せる
- `Start Update Session` で通常の Session Window を開く
- `Extract Memory` / `Copy` で Character Memory を手動利用できる形にする

Character Update Window に置かないもの:
...
### Character Update Window Lifecycle

- `Character Editor` の `Update Workspace` から必要時に生成する
- `characterId` ごとに 1 window を再利用する
- close しても character 自体や Session は削除しない
- `Start Update Session` 後も window は残してよい
...
- `Character Update Window` の生成、再利用、フォーカス
- `Diff Window` の生成
...
flowchart LR
    ...
    CW -->|openCharacterUpdate| MP
    MP -->|create or focus| UW[Character Update Window]
    MP -->|session metadata| SW
    MP -->|character metadata| CW
    MP -->|character metadata| UW
