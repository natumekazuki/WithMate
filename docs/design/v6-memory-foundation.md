...
`forgotten`はfirst releaseではbodyの物理削除や不可逆redactionを意味しない。
hard delete、archive、purge、irreversible redactionは別操作として扱い、foundationのagent-facing APIには公開しない。
...
- forgotten entryのbodyはDBに保持してよい。
...
- bodyの物理削除、不可逆redaction、archive、purgeはfirst release対象外とする。
...
type MemoryForgetResponse = {
  ...
  results: Array<{
    entryId: string;
    status: "forgotten" | "already_forgotten" | "not_found" | "forbidden";
  }>;
};
```
