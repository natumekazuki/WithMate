...
- packaged CLI pathまたはshimをSkillが利用できるようにする。current実装ではSkill内の`bin/withmate-memory.mjs`とWindows向け`bin/withmate-memory.cmd`を同梱し、current raw JSON CLI contractでruntime APIを呼ぶ。
