...
- sampleにはruntime secret、binding reference、discovery file path、internal header / env、local runtime identifierを書かない。
...
- providerごとのbinding capabilityを表示する。current実装ではconfigured providerごとに`env` / `context_file` / `unsupported`を表示する。
...
- runtime API secret、binding reference、discovery documentのsecret値はdiagnostics stateへ含めない。UIにはsecret値を表示しない。
...
Review UIはruntime API secret、binding reference、discovery documentのsecret値をrendererへ渡さず、main process側のReview serviceからV6 Memory storageを扱う。
...
9. providerごとのbinding connection - 完了
10. global Skill install / update - 完了
11. diagnostics - 完了
12. Memory Review UI - 完了
13. optional retrieval enhancement
