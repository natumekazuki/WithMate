...
- `INDEX sessions_lifecycle_updated_idx (lifecycle_status, updated_at DESC)`
- `INDEX sessions_workspace_updated_idx (workspace_key, updated_at DESC)`
...
| `external_conversation_id` | `TEXT` | no | Codex Thread ID または ACP Session ID |
| `persistence_mode` | `TEXT` | no | `persistent` / `ephemeral` |
| `binding_state` | `TEXT` | no | `active` / `invalidated` / `superseded` |
...
- `provider_id` は対応 Session の `provider_id` と一致させる。外部会話 ID は `UNIQUE (provider_id, external_conversation_id)` とし、別 Session へ同じ外部会話を関連付けない。
- `UNIQUE INDEX provider_bindings_one_active_per_session_uq ON provider_bindings(session_id) WHERE binding_state='active'` により、1 Session の active Binding を最大 1 件にする。
- `active` では `superseded_by_binding_id`、`invalidated_at`、`invalidation_reason` をすべて null とする。
- `invalidated` では `invalidated_at` と `invalidation_reason` を必須とし、後継 Binding は持たない。
...
- `created_by_run_attempt_id` は同じ Session に属する Run の Attempt を参照する。`run_attempts.provider_binding_id` との循環参照は Binding 確定 transaction で遅延検証する。
- Binding は最初の Run まで作成を遅延できる。ephemeral Binding も診断用の外部相関として保存するが、process 再起動後の resume には使わない。
...
| `dispatch_state` | `TEXT` | no | `pending` / `dispatching` / `accepted` / `rejected` / `ambiguous` |
...
- `pending` では `dispatching_at` / `resolved_at` を持たない。この状態だけ、未送信を証明して自動送信できる。
- `dispatching` では `dispatching_at` を必須とし、`resolved_at` は持たない。送信 intent の commit 後に Provider request を送る。
- `accepted` / `rejected` / `ambiguous` では `dispatching_at` / `resolved_at` を必須とする。これらは terminal state であり、別 state へ戻さない。
...
Application Service の write、wait、collect 操作に対して、同じ idempotency key の重複実行を防ぎ、確定済み応答を再構築するための軽量な記録を保持する。key は CLI / GUI がアプリ全体で一意な UUID として生成する。

| Column | Type | Null | 意味 |
| --- | --- | --- | --- |
| `idempotency_key` | `TEXT` | no | 全 operation 共通で一意な不透過 key |
| `scope_session_id` | `TEXT` | no | operationの対象または結果を所有するSession。child操作ではchild Session |
...
| `record_state` | `TEXT` | no | `in_progress` / `completed` |
...
- 同じ key と同じ operation / fingerprint の再送は、`completed` なら参照先とenvelopeから同じ意味の応答を再構築し、`in_progress` なら処理中を返す。operationまたはfingerprintが違う場合は `idempotency_conflict` とする。
- domain mutationを伴う短い操作は、domain recordとcompleted IdempotencyRecordを同じtransactionで確定する。長時間待機を伴うwait系操作は先に`in_progress`をcommitし、待機終了後に結果参照とcompleted状態を別transactionで確定する。
- crash後の`in_progress`は成功扱いにしない。operation固有の参照先と副作用を照合し、安全に再評価できるread / waitだけ再開する。mutationの結果を証明できない場合は診断対象とする。
- completed recordの保持中は応答再構築に必要な参照先を削除しない。参照先を先に失効させるpolicyを採る場合は、対応IdempotencyRecordも同じtransactionで失効・削除する。期限切れkeyを新しい操作へ再利用しない。
...
| `payload_state` | `TEXT` | no | `none` / `stored` / `omitted_size_limit` |
...
- `payload_state='none'`では`payload_original_byte_length`をnullとし、payload rowを持たない。`stored`では元byte数を必須とし、同じtransactionのcommit時点で対応するpayload rowを1件持つ。`omitted_size_limit`では元byte数を必須とするがpayload rowを持たない。
...
- `byte_length`は0以上16 MiB以下で、`length(content)`および対応Itemの`payload_original_byte_length`と一致させる。`content_sha256`はcontent bytesのlowercase 64 hex文字とする。
- 1 Runに保存するpayloadの`byte_length`合計は64 MiB以下とする。1件16 MiBまたは累計64 MiBを超えるpayloadはrowを作らず、Itemを`payload_state='omitted_size_limit'`として元byte数とbounded summaryだけ残す。JSON / binaryを部分保存せず、textのpreviewもsummaryの上限内だけにする。
...
1. Provider item ID または WithMate 論理 output ID で重複を検証する。
2. Run 内 ordinal を割り当てる。
3. `run_output_items` に軽量 summary row を追加する。
4. payloadが上限内なら`run_output_payloads`を同じtransactionに追加する。上限超過ならpayload rowを作らずItemを`omitted_size_limit`にする。
5. item と payload を 1 transaction で commit する。

重複Provider itemの再受信は既存output IDを返し、payloadを重複追加しない。payload保存に失敗した場合は`payload_state='stored'`のItemだけを残さない。

### Run terminal

- Run terminal は、確定済み output item の保存を先に収束させる。
...
getRunOutputCounts(runId)
  -> runs の category 別投影だけ

listRunOutputItems(runId, category, afterOrdinal?, limit?)
  -> run_output_items だけ

getRunOutputPayload(outputItemId)
  -> payload_state と、stored の場合だけ run_output_payloads 1 件
```
...
| `closure_reason` | `TEXT` | yes | `completed` / `canceled` / `interrupted` / `abandoned` |
| `created_at` | `INTEGER` | no | 作成時刻 |
| `updated_at` | `INTEGER` | no | 最終更新時刻 |
...
- 許可する遷移は `active -> clarification_required -> active`、`active -> closed`、`clarification_required -> closed` とする。`closed` からは再開しない。
...
2. 新規 instruction なら user `messages` を追加する。exact retry なら元の Message を参照し、追加しない。
3. `runs` に `phase='queued'`で追加し、最初の `run_attempts` を `preparing`、対応する `run_dispatches` を `pending` で作成する。
...
### Provider request の送信

1. Run、Attempt、Binding、Dispatch が同じ Session / Provider に属し、Dispatch が `pending` であることを検証する。
...
- child Run では terminal phase を Delivery に snapshot して `available` とし、Delegation を `closed` と対応する closure reason へ更新する。Provider 実行失敗と結果配送失敗は混同しない。

### child 結果の回収

1. Delivery が `available` で、child Run と参照対象が取得可能であることを検証する。
...
4. 親Sessionがsubtree外に残る場合、subtreeのSession / Run / relation / Delegation / Deliveryをsubjectとする親側RunEventと、それらをresponse refに持つIdempotencyRecordも削除する。RunEvent ordinalの欠番は許容する。child result本文は親MessageやRunOutputItemへ複製保存しないため、親の会話本文や無関係なoutputは削除しない。
...
明示deleteと自動retentionは別操作とする。初期版で確定するのはuserによる明示deleteだけであり、自動retention期間は後続設計とする。
...
3. Attempt と Dispatch を照合する。`pending` だけ自動送信候補にでき、`dispatching` は照会後も受理を証明できなければ `ambiguous` へ進める。`accepted` の外部実行 IDを推測せず、`rejected` / `ambiguous` を `pending` へ戻さない。
4. IdempotencyRecordとdomain recordを照合する。completed refの欠落は応答を捏造せず診断対象にし、`in_progress`を参照先の確認なしにcompletedへ進めない。
5. active Binding は Session ごとに最大 1 件へ収束させるが、外部状態を確認せず invalidated Binding を active に戻さない。
6. child Run と Delivery を照合し、non-terminal Run は `pending`、terminal Run は `available` へ修復する。回収記録は推測で作成しない。
7. Delegation の latest Message / Run は実在する最大の継続組から修復するが、`closed` を `active` へ戻さない。
...
capacity使用数は、`session_relations.orchestration_root_session_id`配下のSessionに属するnon-terminal child Runを`runs`から数えて導出する。root Sessionの上限確認と`queued` Run追加を同じSQLite write transaction内で直列化するため、専用の枠管理tableは持たない。受理済みRunはProvider起動前から1枠を使用し、terminal化した時点で集計から外れる。
...
- `pending` だけを安全に自動送信でき、crash 後の `dispatching` は受理を証明できなければ `ambiguous` へ収束する。
...
- 1件16 MiBまたはRun累計64 MiB超過時にpayload rowを作らず`omitted_size_limit`となり、Run outcome自体は失敗へ変更されない。
...
- 1 Session に active ProviderBinding を 2 件作れず、Binding置換後も旧外部会話 ID と履歴を辿れる。
...
- crash 復旧で `pending` だけを安全に自動送信し、`dispatching` / `ambiguous` を重複送信しない。
...
- wait / collectの応答送信前切断後に、同じkeyで結果参照から応答を再構築できる。
- `in_progress`のcrash復旧でmutation成功を推測せず、安全なread / waitだけ再評価する。
