...
| `payload_state` | `TEXT` | no | `none` / `stored` / `omitted_size_limit` / `omitted_redaction` |
...
- `payload_state='none'`では`payload_original_byte_length`をnull、`redaction_state='not_required'`とし、payload rowを持たない。`stored`では元byte数を必須とし、`redaction_state IN ('not_required', 'redacted')`の場合だけ同じtransactionのcommit時点で対応するpayload rowを1件持つ。`omitted_size_limit`では元byte数と`redaction_state IN ('not_required', 'redacted')`を必須とし、安全性確認済みのbounded summaryだけを残す。`omitted_redaction`では元byte数と`redaction_state='unknown'`を必須とし、payload rowを持たない。
...
3. payloadのredaction要否を判定し、必要ならsanitizerを適用する。判定不能またはsanitizer失敗ならraw payloadを破棄し、Itemを`omitted_redaction`にする。
4. `run_output_items`に安全な軽量summary rowを追加する。
5. 安全性確認済みpayloadが上限内なら`run_output_payloads`を同じtransactionに追加する。上限超過ならpayload rowを作らずItemを`omitted_size_limit`にする。
6. item と payload を 1 transaction で commit する。
...
4. 一意照合できない場合はBindingを`invalidated(invalidation_reason='conversation_start_ambiguous')`、AttemptとRunを`interrupted`へ収束させる。Provider側に残り得る相関不能orphan会話はboundedな診断件数として扱い、自動削除や推測相関を行わない。Provider側retention / cleanupは後続policyで扱う。
...
`waitChild` / `waitAny` / `waitAll`はDeliveryを変更しないread-only待機であり、本transactionを実行しない。複数結果を1つのIdempotencyRecordへ保存しない。結果を初回回収として記録する場合は、対象Deliveryごとに`collectChildResult`を呼ぶ。
...
- `waitChild` / `waitAny` / `waitAll`がDeliveryとIdempotencyRecordを変更せず、複数結果の初回回収はDeliveryごとの明示`collect`だけで記録される。
...
- `creating` Bindingのdurable commit前に外部会話作成requestを送らず、response loss後に同じrequestを自動再送しない。一意照合不能ならBindingとRunがinvalidated / interruptedへ収束し、orphan会話を推測相関・自動削除しない。
