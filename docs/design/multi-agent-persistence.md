...
この決定は旧 DB から新 DB への移行を対象とする。新実装の提供開始後に現行 schema を更新するための versioned migration、初期DDL適用、database識別、schema version管理は`docs/design/sqlite-schema-lifecycle.md`を正本とする。
...
- `active` / `superseded`では`external_conversation_id`を必須とする。`invalidated`では`invalidated_at`と`invalidation_reason`を必須とし、後継Bindingは持たない。外部会話作成requestの未送信または受理不明から失効したBindingは、外部会話IDがnullのまま残ることを許可し、boundedな失効理由codeで両者を区別する。
...
- `UNIQUE INDEX provider_bindings_session_ordinal_uq (session_id, ordinal)`
- `UNIQUE INDEX provider_bindings_external_conversation_uq (provider_id, external_conversation_id) WHERE external_conversation_id IS NOT NULL`
...
- `UNIQUE INDEX run_attempts_run_ordinal_uq (run_id, ordinal)`
- `UNIQUE INDEX run_attempts_binding_external_uq (provider_binding_id, external_execution_id) WHERE external_execution_id IS NOT NULL`
...
| `delivery_state` | `TEXT` | no | `pending` / `dispatching` / `accepted` / `rejected` / `ambiguous` / `aborted` |
...
- `aborted`では`dispatching_at`をnull、`resolved_at`と`resolution_code`を必須とし、Provider未送信のままRunが終端したことを表す。
- terminal delivery state は `accepted` / `rejected` / `ambiguous` / `aborted` とし、別 state へ戻さない。競合は state を条件にした update で検出し、version column は持たない。
...
1. Application Service は admission 直前に Provider capability を再検証する。Persistence Worker は Session、Run、Attempt、Binding の durable state を transaction 内で検証し、supplemental user Message と `pending` Delivery を同じ transaction で durable commit する。いずれかの事前検証で拒否した入力はどちらも作成しない。
...
5. Run terminal transactionは、残っている`pending`を`aborted(run_terminal_not_sent)`、`dispatching`を`ambiguous(process_unknown)`へ進める。Binding失効後に未解決Deliveryを残さない。
6. 明示的な再送は新しい supplemental user Message と新しい Delivery を作る。同じ idempotency key の API / CLI 再送だけは、既存 `idempotency_records` から元の Delivery outcome を返す。
7. `pending` / `rejected` / `ambiguous` / `aborted` Message を後続 Run の initiating Message へ暗黙に転用しない。新しい Run に渡す場合は明示操作で新しい Message と Run admission を作る。
...
| `stored_payload_id` | `TEXT` | yes | `stored`時だけ自身の`id`と同値にし、対応payloadの存在をcommit時に保証する循環参照 |
...
- `payload_state='none'`では`payload_original_byte_length`と`stored_payload_id`をnull、`redaction_state='not_required'`とし、payload rowを持たない。`pending`では元byte数と`redaction_state IN ('not_required', 'redacted')`を必須とし、terminal outcomeをdetail BLOBより先にcommitする間だけpayload rowを持たない。`stored`では元byte数を必須とし、`stored_payload_id=id`と対応payloadへのdeferred foreign keyを同じtransactionで確定する。`redaction_state IN ('not_required', 'redacted')`の場合だけ対応するpayload rowを1件持つ。`omitted_size_limit` / `omitted_persistence`では元byte数と安全性確認済みのbounded summaryだけを残し、`stored_payload_id`とpayload rowを持たない。`omitted_redaction`では元byte数と`redaction_state='unknown'`を必須とし、`stored_payload_id`とpayload rowを持たない。
...
4. 一意照合できない場合は、Run terminal transactionでBindingを`invalidated(invalidation_reason='conversation_start_ambiguous')`、AttemptとRunを`interrupted`、未送信Dispatchを`aborted`へ収束させる。Binding resolution単独ではRunを終端しない。Provider側に残り得る相関不能orphan会話はboundedな診断件数として扱い、自動削除や推測相関を行わない。Provider側retention / cleanupは後続policyで扱う。
5. crash復旧時の`creating` Bindingも同じ規則で照合し、外部会話IDを証明できないまま新しい会話作成requestを送らない。
6. 通常実行中にRunを終端する場合は、Application Serviceが外部会話作成requestの未送信または受理不明を明示する。Run / Attempt、Binding、pending Dispatch、terminal event、child resultを同じtransactionで収束させ、startup repairを通常の完了経路として使わない。
...
- Provider送信前の`creating` Bindingと`pending` Dispatchがある場合は、callerが証明した未送信または受理不明の区別に従って、Run / Attemptと同じtransactionでinvalidated / abortedへ収束させる。`dispatching`または`ambiguous` Dispatchは未送信として処理せず、`not_applicable`でもterminal化しない。BindingとRunのSession、およびBinding ProviderとSession Providerが一致しない場合は更新を拒否する。ephemeral Bindingは作成元Attempt以外からの終端でinvalidatedせず、live ownerも解放しない。
- 同じAttemptのsupplemental inputは、未送信の`pending`を`aborted`、送信intent確定済みの`dispatching`を`ambiguous`へ進めてからRunとBindingを終端する。
...
3. AttemptとDispatchを照合する。Runが`queued` / `starting`、Attempt `preparing`、Binding `active`、Dispatch `pending`の4条件をすべて満たし、Binding、作成元Attempt、Run、SessionのSession / Provider tupleが一致する場合だけ自動送信候補にできる。Binding作成失敗、`canceling`、terminal Runに残る`pending`は、同じtuple内で所有権を確認できる場合だけ`aborted`へ収束させる。`dispatching`は照会後も受理を証明できなければ`ambiguous`へ進める。Provider照会で同じ外部実行IDを一意に証明できた`ambiguous`だけは、Attempt / Runをterminal化する前に`accepted`へ知識補正する。外部実行IDを推測せず、いずれのstateも`pending`へ戻さない。
...
5. creating / active BindingはSessionごとに合計最大1件へ収束させる。作成元Attempt、Run、SessionとのSession / Provider tupleが一致するBindingだけを修復とinspectionの対象にする。persistent Bindingは同じSession内の別Attemptから再利用できるが、ephemeral Bindingは作成元Attemptから参照された場合だけ、修復、inspection、Provider照合候補、recovery projectionのBinding fieldの対象にする。`creating`はProvider照合で外部会話IDを一意に証明できた場合だけactiveへ進め、証明不能ならinvalidatedへ収束させる。外部状態を確認せずrequestを再送したりinvalidated Bindingをactiveへ戻したりしない。
6. terminal Runに残るsupplemental inputは、`pending`を`aborted(run_terminal_not_sent)`、`dispatching`を`ambiguous(process_unknown)`へ収束させる。child Run と Delivery を照合し、non-terminal Run は `pending`、terminal Run は `available` へ修復する。回収記録は推測で作成しない。
...
- previewは最大64 KiBとする。text / JSON / binary chunkはscopeを1024文字以下、要求rangeを256 KiB以下とし、response metadataとtransfer bufferの合計を256 KiB以下へclampする。consumerは実返却byte数で次offsetを計算し、cancel / backpressureを働かせる。binary full BLOBをRendererへhydrateせず明示exportする。
...
- Provider送信前のRun terminalが、callerの未送信または受理不明の証明をDB状態と照合し、Binding / Dispatch / Run / Attemptを同時に収束させる。`dispatching`を未送信として処理せず、通常完了にstartup repairを要求しない。
- Run terminalが同じAttemptの未解決supplemental inputを`aborted`または`ambiguous`へ収束させ、Binding失効後に`pending` / `dispatching`を残さない。
- `pending` / `rejected` / `ambiguous` / `aborted`のsupplemental Messageを後続Runへ暗黙に転用しない。
