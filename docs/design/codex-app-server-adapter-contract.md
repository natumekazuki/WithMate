...
- 状態: 設計の基準（基本通信は実測済み、lifecycle 詳細は一部暫定）
- 調査対象 version: `codex-cli 0.144.1`
...
| 区分 | 意味 | 扱い |
| --- | --- | --- |
| 実測済み | `codex-cli 0.144.1` で通信順序と payload を確認した | 初期実装の基準にできる |
...
- WithMate が `codex app-server` を別 process として起動する。
...
- WebSocket と experimental API は初期実装の必須機能にしない。
...
- ephemeral Thread の `thread/read(includeTurns=true)` は `codex-cli 0.144.1` で拒否された。ephemeral Thread を履歴復旧の根拠にしない。
...
6. `inProgress` を確認し、Runを`active`、Main processのlive activityを`running`へ投影する。
...
stable 生成 schema で agentMessage item は `phase` を持ち、値は `commentary` / `final_answer` / `null` である。schema は Provider が phase を一貫して返すとは限らず、`null` を phase unknown として互換処理するよう求めている。

| App Server item | WithMate 変換 | 状態 |
| --- | --- | --- |
| `agentMessage.phase='commentary'` | RunOutputItem `category='assistant_detail'` | schema 確認。phase 付き runtime は追加検証が必要 |
| `agentMessage.phase='final_answer'` | final assistant candidate | schema 確認。Turn 成功完了まで Message として確定しない |
| `agentMessage.phase=null` | phase unknown item | fallback で分類する。受信時点で final と断定しない |
...
- 解決後の事実は RunEvent、必要な bounded summary は RunOutputItem に保存する。アプリ全体または App Server process の再起動後は、保存済み履歴だけから未解決 request を回答可能な状態へ復元しない。
...
- 受理条件、拒否 payload、履歴への反映は runtime 未検証。

### interrupt

- user cancel を durable に受理し、Run を `canceling` にしてから `turn/interrupt` を送る。
- request 成功だけで Run を `canceled` にしない。
...
- interrupt request から terminal notification までの runtime 順序は未検証。
...
4. persistent Thread / Turn へ一意に再接続でき、resume結果が同じTurnの`inProgress`を示す場合だけ監視を再開する。stdio App Server process異常終了後は`codex-cli 0.144.1`で`interrupted`となるため監視を再開しない。
5. Provider 未送信、terminal outcome、継続可能な外部実行のどれも証明できない Run は `interrupted` にする。

`thread/read` の履歴は照合補助であり、欠落した RunEvent や Message を推測で自動生成しない。切断前に受信済みの未確定assistant deltaがpersistent Turn履歴へ残らないことを確認している。streaming deltaを永続化しない共通方針に従い、crash時の未確定draft消失を許容し、復旧時にpartial outputを推測生成しない。常駐daemonへのclient-only再接続、stdin closeとforce exitの差異は追加検証が必要である。
...
`resumeThread`、`steerTurn`、`interruptTurn`、approval / input response は runtime 検証完了まで capability として条件付きにする。schema に method が存在することだけで、安全に利用可能と公開しない。
...
- daemonを残したclient-only切断と欠落event
...
| 項目 | 現在 | Gate |
| --- | --- | --- |
| persistent Thread resume / read | completed Turnとstdio process異常終了を実測済み | daemonを残したclient-only切断は常駐process設計時に検証 |
| `agentMessage.phase` の runtime 一貫性 | schema 確認のみ | final Message mapper の契約 test |
| `phase=null` fallback | 暫定 | legacy / current model の複数 item 検証 |
| interrupt | schema 確認のみ | cancel mapping 確定前に通信順序を検証 |
| steer | schema 確認のみ | accepted / rejected / ambiguous を検証 |
