...
- V6 foundation schema source:
  ...
  - Work Itemは`root` / `delegated` unionとしてSession binding、planned source identity、状態revisionを`work_items_v6`へ保存する。rootは`standalone` / `overall-coordinator` root Sessionごとに一件だけで、Session rowと同じtransactionで作成する。delegatedのassignmentはreassign/moveで履歴付きに変更でき、clone/reopenは新しいIDを発行して旧Work Itemのresult・decision・所属履歴を保持する。moveのadoptionは結果の自動採用ではない。同一root内のmoveは影響する確定済みparent/ancestorの集約をstaleにし、判断の再確認と訂正結果の再確定を必要とする。cross-rootのWork Item単体moveは移管先targetのcurrent revisionと双方のgrantを検証し、root・creator・targetと所属eventを同一transactionで更新する。集約descendantを持つWork Itemの単体移管は拒否し、旧履歴と累積予算消費を保持する。24時間expiry付きmutation replayは`work_item_idempotency_v6`、executionとの別identity associationは`work_item_execution_associations_v6`へ保存し、result terminal tupleと256 KiB上限はDB CHECKでも保持する。idempotency responseはevent単体ではなくcanonical Work Item全体を保存するため2 MiBを上限とし、migration前のresponseがないrowの`NULL`は現在値へのfallbackではなくresponse復元不能を表す。rootのcurrent projectionとappend-only履歴は単調増加revisionで直列化する。通常event payloadは512 KiB、V1 public contractのcurrent projectionを保持する`migration_baseline`は2 MiBを上限とし、event種別ごとのDB CHECKで制限する。archiveは履歴を保持したまま既定一覧から隠し、restoreはarchive状態を戻す。deleteは適格なterminal Work Itemのcurrent rowを物理削除し、履歴、idempotency response、execution association、aggregation ledgerとtombstoneを保持する。migration時はdelegated baselineから開始する
  - root全体の移管ではterminal Root Work Itemを同一IDのdelegatedへ変換し、`origin_kind = 'transferred_root'`を保存する。通常rowの既定値は`native`であり、移管由来のterminal rowだけがRoot Workの空契約・進捗を保持できる。既存DBはWork参照tableの全列・index・triggerを保持してCHECKを更新し、過去event・result・再送snapshotを再baselineしない。公開projectionは移管由来のrowにだけ`originKind`を付ける。
