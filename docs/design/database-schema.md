...
- `source_schema_version < 5` または `access_mode = 'legacy_readonly'` の agent session は旧互換 session として扱い、Home では `Read Only` 表示にする。履歴閲覧のため Session Window は開けるが、update / send / model 変更 / approval 変更などの永続更新は拒否する
