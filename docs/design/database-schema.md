...
lifecycle は V1 の `SessionStorage` / `AuditLogStorage` を生成せず、`SessionStorageV2Read` / `AuditLogStorageV2Read` を V2 storage として使う。V2 runtime write-path は session / audit log の既存 write-capable method を V2 split schema へ保存する。
