...
lifecycle は V1 の `SessionStorage` / `AuditLogStorage` を生成せず、`SessionStorageV2Read` / `AuditLogStorageV2Read` を read adapter として使う。V2 runtime write-path はまだ未切替のため、session / audit log の write-capable method は V2 read-only storage に対して明示エラーにする。
...
V2 read-path は既存 IPC / renderer contract を維持するため、V2 split table から既存 `SessionSummary` / `Session` / `AuditLogEntry` shape を復元する。V2 には monologue legacy `stream_json` がないため、`Session.stream` は `[]` として復元する。既存 audit log IPC は現時点で `AuditLogEntry[]` を返すため、audit log read adapter は当面 `audit_log_details` も復元する。audit log pagination / lazy detail API は後続設計で扱う。
