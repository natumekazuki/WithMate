...
- V6 foundation schema source:
  ...
  - Work ItemはimmutableなSession bindingと状態revisionを`work_items_v6`、mutation replayを`work_item_idempotency_v6`、executionとの別identity associationを`work_item_execution_associations_v6`へ保存する。result terminal tupleと256 KiB上限はDB CHECKでも保持する
