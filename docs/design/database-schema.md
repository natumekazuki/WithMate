...
- V6 foundation schema source:
  ...
  - Work Itemは`root` / `delegated` unionとしてimmutableなSession bindingと状態revisionを`work_items_v6`へ保存する。rootは`standalone` / `overall-coordinator` root Sessionごとに一件だけで、Session rowと同じtransactionで作成する。24時間expiry付きmutation replayは`work_item_idempotency_v6`、executionとの別identity associationは`work_item_execution_associations_v6`へ保存し、result terminal tupleと256 KiB上限はDB CHECKでも保持する。idempotency responseはevent単体ではなくcanonical Work Item全体を保存するため2 MiBを上限とし、migration前のresponseがないrowの`NULL`は現在値へのfallbackではなくresponse復元不能を表す。rootのcurrent projectionとappend-only履歴は単調増加revisionで直列化する。通常event payloadは512 KiB、V1 public contractのcurrent projectionを保持する`migration_baseline`は2 MiBを上限とし、event種別ごとのDB CHECKで制限する。Session削除はactiveまたは未回収のWork Itemを保護し、parent-null resultは同一root SessionにterminalなRoot WorkItemが存在する場合だけ回収済みとする。terminalの肯定的証拠がない場合は削除を拒否し、terminalかつ回収済みのdelegated Work Itemと関連ledgerを同じtransactionで物理削除する。migration時はdelegated baselineから開始する
