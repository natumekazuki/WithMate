...
- 初期DDLをschema version `1`とする。初回リリース前は開発databaseを互換対象とせず破棄可能とするため、version 1 artifactを現行契約へ更新できる。初回リリース後は、table、column、index、trigger、制約または保存形式を変更するたびに整数を1ずつ増やす。
- 初回リリース後の各versionは直前versionからのmigrationを1つだけ持つ。versionの飛び越し、分岐、同じversion番号のDDL差替えを禁止する。
...
| 新規databaseへ現行schemaを作る完全DDL | schema version 1のDDL artifact |
...
初回リリース後にversion 2が必要になった時点でmigration manifestと`1 -> 2` artifactを追加し、migration失敗・再開を検証する。
