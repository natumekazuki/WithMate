...
本書はdatabase fileのlifecycleとschema適用手順を定める。14 tableの列、制約、index、transaction境界の正本は`docs/design/multi-agent-persistence.md`とし、本書へ重複させない。
...
- 初期DDLをschema version `1`とする。以後は互換性のない意味変更だけでなく、table、column、index、trigger、制約または保存形式を変更するたびに整数を1ずつ増やす。
...
3. 1つのwrite transactionで14 table、index、必要なtriggerを作成する。
...
| 関心事 | 正本 |
| --- | --- |
| 14 tableのcolumn、FK、CHECK、UNIQUE、index | `docs/design/multi-agent-persistence.md` |
...
3. version 2が必要になった時点でmigration manifestと`1 -> 2` artifactを追加し、migration失敗・再開を検証する。
