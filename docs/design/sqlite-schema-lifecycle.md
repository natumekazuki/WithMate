...
- 旧WithMate DBのschemaを入力にしたmigration test
...
- 定数値は`schema/sqlite/manifest-v1.json`をmanifest上の正本とし、変更しない。DDLとcontract testは同じ値を固定する。
...
- 初期DDLをschema version `1`とする。新実装を含む最初の正式release以後は、互換性のない意味変更だけでなく、table、column、index、trigger、制約または保存形式を変更するたびに整数を1ずつ増やす。
- release済みの各versionは直前versionからのmigrationを1つだけ持つ。versionの飛び越し、分岐、同じversion番号のDDL差替えを禁止する。
- applicationは`currentSchemaVersion`と`minimumSupportedSchemaVersion`を持つ。初期実装では両方を`1`とする。
...
### 正式release前のversion 1

新実装を含む最初の正式releaseまでは、version 1のDDLとmanifestを同じ論理変更で更新できる。この期間に開発buildが作成したdatabaseは互換性保証の対象外とし、schema hashが一致しない場合は開発者が明示的に削除して再作成する。applicationは自動削除しない。旧WithMate DBはこの例外の対象ではなく、引き続き参照、変更、自動削除を行わない。

最初の正式releaseに含めたversion 1を凍結点とする。凍結後のschema変更はversion 2以降とし、直前versionからのmigrationと回帰testを追加する。
...
3. 1つのwrite transactionでmanifestに列挙したtable、index、必要なtriggerを作成する。
4. 同じtransaction内で`user_version=1`を最後に設定する。
...
`schema/sqlite/v1.sql`はtable、index、triggerだけを定義する純粋なschema artifactとする。header PRAGMA、write transaction、`user_version`、commit後検証、WALとconnection PRAGMAはPersistence Workerが所有する。これによりtransaction途中の失敗をrollbackし、schema検証前にWALへ切り替わることを防ぐ。
...
| 関心事 | 正本 |
| --- | --- |
| 現行tableのcolumn、FK、CHECK、UNIQUE、index | schema version 1のDDL artifactとmanifest |
| 初期schemaを作る完全DDL | schema version 1のDDL artifact |
...
- 空databaseへのbootstrapがversion 1の完全schemaを作り、2回目の起動がDDLを再適用しない。
...
1. `schema/sqlite/v1.sql`と`schema/sqlite/manifest-v1.json`をPersistence Workerのbootstrapから適用する境界を設計する。
2. `scripts/validate-sqlite-schema.py`の代表検証をtest runnerへ統合し、各tableの検証Gateをcontract testへ拡張する。
3. 最初の正式release後にversion 2が必要になった時点でmigration manifestと`1 -> 2` artifactを追加し、migration失敗・再開を検証する。
