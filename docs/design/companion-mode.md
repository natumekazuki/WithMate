...
- Companion Mode の検討過程と詳細論点は archived planning notes を参照する
...
Current MVP 実装では、`companion_groups`、`companion_sessions`、`companion_messages`、`companion_merge_runs` を追加し、CompanionSession 作成、base snapshot ref / shadow worktree 作成、Home での active / history 一覧表示、history card の selected files summary / changed files summary / sibling warning summary、CompanionSession から shadow worktree 上で provider 実行する最小導線、Review Window での changed files / split diff / merge readiness / merge run timeline 表示、terminal CompanionSession の read-only Review Window 表示、selected files merge / discard、merge 後の sibling path overlap warning、merge / discard 完了時点の diff snapshot 永続化までを扱う。review window 用の詳細 table は後続実装で追加する。
...
- snapshot commit は `refs/withmate/companion/<sessionId>/base` に保持する
...
- snapshot ref: `refs/withmate/companion/<sessionId>/base`
...
Current MVP 実装の Review Window は、Home の active CompanionSession から開く補助 window とし、base snapshot commit と shadow worktree の差分を都度読み取って changed file list と split diff を表示する。changed file list は default selected とし、user は file 単位で merge 対象を外せる。`Merge Selected Files` / `Discard Companion` は実行でき、target branch drift、target workspace dirty、merge simulation の結果を merge readiness として表示する。merge 後は同じ CompanionGroup の active sibling CompanionSession と selected files の path overlap warning を表示する。merged / discarded / recovery-required の terminal CompanionSession は Home の history card として表示し、history card から read-only Review Window を開ける。terminal Review Window は `companion_merge_runs` の latest run を優先して changed file summary と diff snapshot を表示し、同 session の merge / discard 履歴を timeline として表示する。古い履歴など diff snapshot がない場合は changed file summary と empty diff rows に fallback する。checks 表示は後続実装で追加する。
...
Current MVP 実装の sibling check は temporary sibling check ref を作らず、merge 後の結果として Review Window に返し、terminal CompanionSession の `sibling_warnings_json` にも保存する。判定は selected files と active sibling CompanionSession の changed files の path 一致に限定し、overlap がある場合は warning を表示する。sibling check 自体に失敗しても selected CompanionSession の merge は完了させ、warning として user に返す。
...
Current MVP 実装では、terminal CompanionSession を既存 `companion_sessions` の `status`、`updated_at`、`selected_paths_json`、`changed_files_json`、`sibling_warnings_json` で保持し、Home の Companion 欄に history として表示する。merge / discard の terminal 操作は `companion_merge_runs` にも completed history として保存し、Home history card は latest merge run を優先して summary を表示する。terminal read-only Review Window は latest merge run を changed file summary と diff snapshot に使い、同 session の merge runs を newest-first timeline として表示する。
