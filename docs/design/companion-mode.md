...
Current MVP 実装では、terminal CompanionSession を既存 `companion_sessions` の `status`、`updated_at`、`selected_paths_json`、`changed_files_json`、`sibling_warnings_json` で保持し、Home の Companion 欄に history として表示する。merge / discard の terminal 操作は `companion_merge_runs` にも completed history として保存する。
