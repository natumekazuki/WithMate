...
Current MVP 実装では、`companion_groups`、`companion_sessions`、`companion_messages`、`companion_merge_runs` を追加し、CompanionSession 作成、base snapshot ref / shadow worktree 作成、Home での active / history 一覧表示、history card の selected files summary / changed files summary / sibling warning summary、CompanionSession から shadow worktree 上で provider 実行する最小導線、Review Window での changed files / split diff / merge readiness 表示、terminal CompanionSession の read-only Review Window 表示、selected files merge / discard、merge 後の sibling path overlap warning までを扱う。review window 用の詳細 table は後続実装で追加する。
