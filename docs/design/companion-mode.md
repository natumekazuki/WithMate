...
Current MVP 実装では、`companion_groups` と `companion_sessions` を先に追加し、CompanionSession 作成と Home での active 一覧表示までを扱う。snapshot / shadow worktree 実体作成 / merge / discard / review window 用の詳細 table は後続実装で追加する。
