...
- model / depth は catalog から選択し、実行前に provider catalog で検証する。保存・APIへ渡す raw ID は変更せず、Depth の表示は catalog の表示label（`Minimal` / `Low` / `Medium` / `High` / `X High` / `Max` / `Ultra`）へ変換する
