...
- **確定方針 / 今後反映**として、catalog import 時は既存 session も新 active revision へ自動 migrate する
...
- migration の具体的な正規化ルールは別途実装設計で詰めるが、少なくとも `import 後に自動 migrate される` 前提は本書の正本とする
...
- **確定方針 / 今後反映**として、import 成功時は既存 session の `catalogRevision` と必要な model / depth 解決も自動 migrate する
- partial merge はしない
