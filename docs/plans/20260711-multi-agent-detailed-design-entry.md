...
- 子は複数・入れ子で起動でき、同時実行上限は `orchestrationRootSessionId` 配下の全子孫で共有し、全 child Run start が Run 単位の capacity admission を通る。
...
- initiating user Message を正本とする Delegation / Run の参照、ChildResultDelivery、evaluation record の table と参照整合性
- root 単位の capacity admission を直列化する transaction / actor 境界
- initial / resume / retry の child Run admission、capacity reservation の durable commit、Provider 起動の順序
- terminal Run、capacity reservation release、Session projection、Delegation、delivery state の同時更新境界
...
- 全 child Run admission と terminal capacity reservation release の transaction sequence
