...
- 優先度が高い UI virtualization を小さく実装し、Renderer 側の過剰描画を抑える。
...
- Audit Log モーダルの一覧描画を windowing し、既存の paging / detail lazy load と両立させる。

## スコープ外

- 既存ユーザーデータの破壊的変更。
- 本番起動時の自動 migration。
- Message 一覧の virtualization 本体の実装。
- 長期保持ポリシーの確定。
...
- [x] Audit Log モーダルの virtualization を実装する。
- [x] Audit Log virtualization の回帰テストを追加する。
- [x] 必要な検証を通す。
- [ ] 結果を記録して archive する。
...
- Audit Log 側は既存の paging と detail lazy load があり、Message 側より scroll/follow 状態への影響が小さいため先に実装する。
