...
- plan tier: repo plan
- 状態: complete
- 現在地: S8とCP1完了。次はCP2 Application Service / CLI Control Plane
...
| S6    | Repository Write Transactions          | 完了                      | admission、terminal、output、binding / dispatch、delivery、delete、repair          | S4、S5 |
| S7    | Contract / Concurrency / Fault Tests   | 完了                      | schema test統合、競合、rollback、crash、shutdown、payload検証                      | S3〜S6 |
| S8    | CP1 Integration Gate                   | 完了                      | public API review、dependency guard、full CP1 validation、docs同期                 | S7     |
...
- B: Run admission、Attempt / Binding / Dispatch、supplemental input（B1 通常Run admission、B2 Binding / Dispatch transition、B3 retry / supplemental input、B4 initial child admission完了。Auxiliary固有policyはCP5）
- C: RunOutput、terminal、child result collect（完了）
- D: Session subtree delete（durable cleanup manifestを含む）、startup repair（完了）
