...
- plan tier: repo plan
- 状態: active
- 現在地: S6-D進行中、Session subtree delete実装済み。次はstartup repair
...
| S6    | Repository Write Transactions          | 進行中（D repair残）       | admission、terminal、output、binding / dispatch、delivery、delete                  | S4、S5 |
...
- D: Session subtree delete（durable cleanup manifestを含め実装済み）、startup repair（残）
