...
| S3 | SQLite Bootstrap / Schema Verification | 次に着手 | DB分類、fresh bootstrap、manifest検証、非対応DB拒否 | S2 |
| S4 | Persistence Worker Lifecycle | 未着手 | Worker起動、request protocol、write queue、read、shutdown、crash処理 | S3 |
| S5 | Repository Read Model | 未着手 | Session / timeline / Run / output / child resultのbounded query | S4 |
| S6 | Repository Write Transactions | 未着手 | admission、terminal、output、binding / dispatch、delivery、delete | S4、S5 |
