...
source root budgetは同じaccount IDのsession allocationへ変換する。source直下のallocationはdestination root account直下へ移し、それより深いaccountの親は維持する。累積消費・履歴は保持し、共有storageのcommittedだけをdestination rootへ移送してsource accountのstorage limit・committedを0にする。destinationのhard limitを自動拡張せず、容量不足、未精算reservation、移管先deadlineを超えるallocationを拒否する。
