...
- root coordinatorが作成するtop-level delegated Work Itemはparentを持たない。parentを指定できるのはactor Sessionがtargetであるactiveなdelegated Work Itemだけとし、Root Work Itemはdelegation parentにもaggregation parentにもならない。Root finalizationはparent-null top-level branchとdelegated parent配下のnested aggregationを別のclosureとして判定する。
...
- 通常eventのpayloadはUTF-8 JSONで512 KiB以下とし、exact payloadが確定するmutation境界とDB CHECKで検証する。V1 public contractから生成できる既存値を欠落なく移すため、`migration_baseline`だけは2 MiB以下を許可する。baselineはlegacy revisionに対応する一件のeventとし、分割用のsynthetic revisionを生成しない。
...
- 履歴consumerは通常event上限とmigration baseline上限をruntime catalogから区別できる。
...
### migration baselineを複数eventへ分割する

同一revisionの複数eventまたはsynthetic revisionが必要となり、既存のrevision一意性とaggregation decisionのchild revisionを変更するため採用しない。V1 public contractの最大snapshotを収容できるbaseline専用上限を使う。
