...
- 左右 splitter と上下 splitter の pointer / click / keyboard 操作は対応する領域の開閉または resize を続行する
- wide layout では Header と ActionDock を全幅に通し、side pane は中央 surface の横に配置する
- narrow layout では表示中の左右paneと中央surfaceの縦stackを維持し、中央surfaceが 0px になる状態も許容する。左右paneのサイズは縦dragと上下矢印キーで調整する
- Header は1行分の固定高とし、高さ変更は受け付けない
- ActionDock は完全には非表示にせず、compact state でも draft、添付数、run 状態、末尾移動、Send / Cancel に必要な最小情報を残す
- wide layout の ActionDock は上部操作列と下部設定・送信列を固定し、中央の textarea 領域だけを伸縮させる。中央の chat / preview surface は必要に応じて 0px まで縮小できる
...
- Header と ActionDock が常に Window 全幅を使い、side pane と中央 surface の配置が一定になる
