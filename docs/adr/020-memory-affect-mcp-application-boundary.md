...
- post-turn appraisalは評価直前の最新contextを使い、評価したcandidate列、expected Affect version、評価世代、観測済みeffectと進捗をpending recordへ保存してからwriteする。`effect: none`のversion conflictは、その評価世代のidempotency keyがcommitされていないことを確認できるため、同一transactionで次のdurable評価世代へ進め、最新contextから再評価する。評価世代ごとに新しいidempotency namespaceを使い、同じkeyへ異なるcandidateを再割当てしない。1回のsettlement処理は一つのappraisal、1回のdrainは固定件数までとする。pendingが残る場合はlifecycle ownerが重複しない遅延drainを予約し、競合や一時障害が続く間は上限付きbackoffで再処理する。process終了時は予約を止め、次回起動時にdurable pendingから再開する。`partial`、`committed`、`unknown`の後は保存済みcandidate identityを変更せず、同じidempotency keyでread-backまたはreconcileする。ただし、そのreconcileが`effect: none`のversion conflictを返した場合は未commitが確定するため、同じ次世代遷移を適用する。turn correlationと初回Affect eventのidempotencyは表示用timestampではなく永続audit IDを使う。
