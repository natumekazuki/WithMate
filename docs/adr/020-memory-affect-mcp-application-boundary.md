...
- runtime discoveryはCLI用とMCP用の投影を分け、共通connectionはbase URL、通常secret、runtime instanceだけを保持する。CLI投影はoperator credential、MCP投影はMCP credentialだけを持つ。generation固有fileを先にpublishし、adapter別のcurrent pointerをatomic renameで差し替える。停止時は自runtimeのgenerationだけを削除し、current pointerをread後に削除する所有判定は行わない。
