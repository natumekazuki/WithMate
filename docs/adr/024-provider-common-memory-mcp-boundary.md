...
- agent-bound CLI fallbackは、同じprovider executionがMCP initializeに成功し、`tools/list` responseを正常に送出した後に発生したtransport-level availability failureだけを開始条件とする。MCP processは通常のMCP operation credentialとは別にruntime registryが当該generationへ発行するfallback admission reporter credentialを読み、`tools/list`成功responseをtransportへ渡す前にruntime generation、binding、turn capabilityへ結び付く短命な`listed` stateをserverへ登録する。serverは各`listed` registrationへopaque admission tokenを結び付け、MCP processはresponse送出成功時に確定したtokenを対応contextと一体で保持する。response送出中は後続operationのeligible遷移を送出結果の確定まで保留し、送出に失敗した場合はserverが新規登録時だけ発行したrollback tokenで同じ`listed` stateを削除する。既存stateに対する重複`tools/list`は同じadmission tokenを返すがrollback tokenを発行せず、失敗した重複responseから先行stateを削除または再武装できない。その後の実transport exception時だけ、同じreporter credential、正常送出済みregistrationのadmission token、method、path、bodyのfingerprintを持つ`eligible` stateへ遷移させる。Memory operationは開始時点の正常送出済みadmissionを捕捉し、後発の未送出registrationへtokenを差し替えない。通常のMCP credentialだけではcontrol stateを作成または更新できない。state transitionは`listed`から`eligible`、`eligible`から`consumed`への一方向とし、CLI requestは同じbinding、current turn、operation fingerprintに一致するadmissionだけを原子的にconsumeできる。同じfingerprintのresponse loss retryを除いて再利用できず、`consumed`から別fingerprintへ再武装できない。変更request、期限切れ、stale turnはdispatch前に拒否する。
