...
- Windowの常設操作はSession選択とstate filterに限定し、画面内title、説明文、Home遷移、取得済み件数を置かない。Session pickerとevent一覧は末尾接近で自動的に次pageを読み込み、一覧とdetailを独立してscrollできるようにする。回答mutation中は二重操作を防ぎ、再読込後も選択中Eventを維持する。
