...
- System Promptは登録対象、保存禁止情報、登録失敗時の扱い、作成・list結果の`eventId`による対象指定、作成response喪失時のidempotency keyによる再取得、未使用回答を反映した後の`consume`を固定指示として説明する。回答本文はSystem Promptへ含めず、Conversation Timingの後かつUser Inputの前にinput contextとして投影する。未使用回答がないTurnでも固定System Promptを変えず、prompt complianceはauthority、validation、idempotencyの代替にしない。
