...
- payloadはsummaryと限定されたdetail fieldだけを受け付け、UTF-8 JSON 16 KiB、field件数・文字数を共有validatorで検証する。secret、raw log、stack trace、大きなdiff、provider response、chain-of-thought、個人環境path、opaque bindingを拒否する。execution関連付けはactor Session所有のexecutionだけを許可する。
