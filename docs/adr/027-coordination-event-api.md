...
- System Promptは登録対象、保存禁止情報、登録失敗時の扱い、未使用回答を反映した後の`consume`をprovider共通の固定指示として説明する。API入出力、`eventId`による対象指定、idempotency keyによる再取得などの操作方法はtool schemaとdescriptionを正本とし、System Promptへ重複させない。回答本文はSystem Promptへ含めず、Conversation Timingの後かつUser Inputの前にinput contextとして投影する。未使用回答がないTurnでも固定System Promptを変えず、prompt complianceはauthority、validation、idempotencyの代替にしない。
