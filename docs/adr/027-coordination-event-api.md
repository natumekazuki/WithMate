...
- Coordination UIはSession右ペインではなく、全Sessionのeventを扱う単一の`Coordination Window`へ集約する。初期表示はSessionとstateを絞らない「すべて」とし、新しいeventから取得する。`open`を先に並べ替えず、要回答、未解決、回答済み、履歴はユーザーが選ぶfilter projectionとして提供する。
...
- 登録対象、kindの区別、保存禁止情報、登録失敗時の扱いなどの静的な利用規則はMCP server instructionsとtool descriptionを正本とし、System Promptへ重複させない。未使用回答があるTurnだけ、回答本文と反映後の`consume`指示をConversation Timingの後かつUser Inputの前にinput contextとして投影する。prompt complianceはauthority、validation、idempotencyの代替にしない。
