...
- provider clientはbinding generationとlogical turn capabilityのhashをcache identityへ含め、Session/provider/turn単位で分離する。同じlogical turn内のretryは同じclient-scoped environmentを再利用し、次turnではbinding generationを維持したまま新しいclientを作る。provider executionを再生成する場合は旧bindingも失効させる。
- binding generationはprovider executionのidentityであり、logical turnのidentityには使わない。`SessionRuntimeService`はprovider呼出直前のbeginと終了時のendというlease lifecycleを所有し、genericな`ProviderAgentRuntimeTurnCoordinator`はactor Session/providerごとのprocess-local active lease、推測不能なturn capabilityの発行、照合、失効を所有する。Glossary proactive createとMemory/Character mutation、Memory file exportは同じcoordinatorの兄弟consumerとし、capabilityの別mapを持たない。同一turn内のstale thread retryでは同じcapabilityを維持し、turn終了で失効させる。前turnの遅延child requestが次turnのleaseへ到達しても、capability不一致で副作用前に拒否する。
- `process.env`は変更しない。defined environment、binding reference、turn capability、およびprovider executionではbinding省略を許さない非secret markerをprovider client作成時に明示的に渡す。managed MCP/CLI runtime clientはmarkerがあるのにreferenceが欠落した場合、optional Memoryをlocal-userへfallbackせずdispatch前に失敗する。外部terminalにはmarkerとturn capabilityを渡さず、別の認証と明示targetによるoptional経路を維持する。
- binding-requiredなMemory経路では、provider executionごとのclient-scoped environmentへ`applicationInstanceId`とMemory固有の`runtimeGenerationId`を非secret owner selectorとして投影する。`process.env`へ書き込まず、provider client cache identityへMemory generationを含め、background/unbound clientへ残ったselectorは大小文字を区別せず除去する。selectorが欠落または不一致の場合はlocal-user/operator経路へfallbackせず、dispatch前に失敗する。Session runtime generationとの一致は要求しない。
...
- provider child runtimeにはopaque referenceとturn capabilityをclient-scoped environmentとして渡す。providerが同じenvironmentで起動するchild processからの秘匿は保証せず、同一OS user上の攻撃的なprocessは本境界のthreat model外とする。WithMateは両secretをprompt、UI、通常log、provider設定へ投影せず、third-party MCPへruntime credentialやWithMate MCP設定を自動付与しない。
- managed WithMate MCP/CLI runtime clientは、同一HTTP接続上のidentity challengeを検証した後、exchange requestのJSON envelopeへenvironmentのreferenceとturn capabilityを格納する。challenge前のrequest、通常operation body、MCP transport session IDはactor identityに使用しない。
- agent runtime extension requestは、同じidentity challengeと認証envelopeを使う専用exchangeへ送る。専用exchangeはMemory operationへfallbackせず、独立したbody上限と同時実行上限を持つ。Memoryの通常exchangeはrequest sizeにかかわらずagent runtime extensionをdispatchせず、通常exchangeとdirect routeの上限は拡張しない。
...
- opaque reference、turn capability、runtime secret、binding hash、operation grant detailはprompt、UI、通常log、public errorへ投影しない。provider outputから作るlive state、監査raw item、metadata、artifact、public errorは、boundingや永続化より前に現在のbinding referenceとturn capabilityの完全一致を固定markerへ置換する。providerへ送るuser input、logical prompt、transport payload、provider control identifierはこのprojection redactionの対象にしない。
