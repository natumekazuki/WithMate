...
- binding generationはprovider executionのidentityであり、logical turnのidentityには使わない。turn単位の制約は`SessionRuntimeService`がprovider呼出の直前から終了まで保持するprocess-local leaseと、leaseごとの推測不能なturn capabilityで管理する。同一turn内のstale thread retryでは同じcapabilityを維持し、turn終了で失効させる。前turnの遅延child requestが次turnのleaseへ到達しても、capability不一致でmutation前に拒否する。
