...
- agent runtime extensionの大きなrequestは、同じidentity challengeと認証envelopeを使う専用exchangeへ送る。専用exchangeはMemory operationへfallbackせず、独立したbody上限と同時実行上限を持つ。Memoryの通常exchangeとdirect routeの上限は拡張しない。
