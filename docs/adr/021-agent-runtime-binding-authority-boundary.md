...
- binding generationはprovider executionとcacheのidentityであり、logical turnのidentityには使わない。turn単位の制約は`SessionRuntimeService`がprovider呼出の直前から終了まで保持するprocess-local leaseで管理し、同一turn内のstale thread retryでは同じleaseを維持する。
