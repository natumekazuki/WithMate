...
Application contractはPersistence Worker固有のrequest optionやerror codeを公開しない。timeout / cancel optionはApplication operation全体へ適用するApplication所有型で受け、access validation中もdeadlineまたはcancelでsettleさせる。Repository開始前の中断はoperation errorと`not_attempted`、開始後の中断はpersistence failureとし、writeのcommit結果が不明なら`unknown`を維持する。persistence errorはunavailable、busy、timeout、canceled、configuration、integrity、response size、operation failureの安定codeへ写像し、failure effectとretryableを失わない。

authorization contextはgenericなoperation contextとして受け、workspace validationとauthorizationを行う注入portへ渡す。認証方式固有の値を`structuredClone`可能と仮定せず、composition rootが注入するauthorization snapshot policyでrequest decodeとvalidatorごとの独立viewを作る。Application Serviceは両方のvalidationをRepository呼び出し前に実行し、Persistence Workerへ認証主体を渡さない。
