...
authorization contextはgenericなoperation contextとして受け、workspace validationとauthorizationを行う注入portへ渡す。認可入力にはworkspaceだけでなく、operationごとのSession targetまたはcreate設定を含め、Session単位と追加directory単位のpolicyをRepository呼び出し前に判定可能にする。Session作成の追加directoryはhost OSで完全修飾されたpathだけを受理し、shared canonicalizationをrequest decode直後に一度適用して、認可対象とRepository commandを同一値にする。認証方式固有の値を`structuredClone`可能と仮定せず、composition rootが注入するauthorization snapshot policyでrequest decodeとvalidatorごとの独立viewを作る。Application Serviceは両方のvalidationをRepository呼び出し前に実行し、Persistence Workerへ認証主体を渡さない。

Session追加directory設定の本文はinline / chunkedにかかわらず汎用detailへ含めず、workspaceとSession targetを再認可するApplication read operationから取得する。CLIやGUIへRepository clientの迂回利用を要求せず、本文の公開可否をpayload sizeで変えない。
