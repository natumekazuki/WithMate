...
executable名は`withmate`、Session namespaceは`withmate session <operation>`とする。現行operationは`create`、`list`、`read`、`directories-chunk`、`archive`、`unarchive`、`close`であり、CLI handlerは注入された`ApplicationSessionOperations`だけを呼び出す。
...
Session IDを直接指定するoperationではWorkspaceを要求しない。createだけがWorkspaceのabsolute pathを受け、Application側がSessionに保持する。listはアプリ全体のSessionを既定scopeとし、Workspaceとlifecycleを任意filterとして受ける。現行Application contractには独立したSession search operationを追加せず、linked / child Session searchは後続scopeとする。
