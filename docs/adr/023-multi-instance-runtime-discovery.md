...
- cleanupは自分のidentity tuple（application instance、runtime generation、adapter kind）とgenerationだけを対象とする。正常終了はunpublish、listener停止、自generation削除の順序で行い、ownerでないruntimeのcleanupはactive集合を変更しない。entry commit後のlock解放失敗などで結果が不明な場合は同じidentity tupleとpublication IDをread-backし、callerへhandleを返せないpublicationはowner entryをrollbackしてから失敗を返す。lock directoryが残る解放失敗では、そのdirectoryが他processを排他している間にrollbackとadapter projection復元を完了し、別ownerのartifactは変更しない。
...
- legacy pointerのpublishとcleanupはMemory adapter所有のprocess間lockで直列化する。新しいMemory entryのpublish前に同じregistry mutation lock内でpointerを非公開にし、publication集合の検証とpointer commitもそのlock内で一体化する。entry publicationがrollbackされた場合は同じlock順で直前のpointerを復元する。cleanup対象generationをpointerが指す場合、またはpointerが未公開の場合は、canonical registry entry、identity challenge、CLI/MCP両generation documentが一致する生存候補が一意なときだけ、そのgenerationへatomic handoffしてから自generation pairを削除する。freshなentryはcredential状態にかかわらずactive集合へ含め、候補が0件、複数、または選択後に集合が変化した場合はpointerを公開せず、起動順やlast writerで暗黙選択しない。pointerが別generationへ更新済みなら変更しない。
