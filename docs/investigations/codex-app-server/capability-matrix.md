...
| assistant phase | agentMessage `phase=commentary` / `final_answer` / `null` | 仕様・schema 確認 | final Message と assistant detail の分離に使う。`null` fallback は runtime 検証が必要 |
...
- CAS-017の常駐daemonを残したclient-only切断、stdin closeとforce exitの差異
- 同一 process 上の複数 Thread / Turn の event 相関と並行性
- `modelProvider/capabilities/read` の version 差分
- agentMessage `phase` が付く場合と `null` の場合の item 順序、final response との対応
