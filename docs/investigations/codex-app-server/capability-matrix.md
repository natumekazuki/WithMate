...
- 調査日: 2026-07-10、2026-07-20
- 対象 version: `codex-cli 0.144.6`（persistent Thread復旧の既存実測は`0.144.1`）
- 関連設計: `docs/design/provider-integration.md`, `docs/design/codex-app-server-adapter-contract.md`
- 状態: CP3 runtime contract確定済み。interaction詳細と並行実行は継続調査
...
| managed daemon lifecycle | `codex app-server daemon` | Windowsでblocked | 初期実装の必須依存にせず、runtime host自身がProvider childを所有する |
...
| assistant phase | agentMessage `phase=commentary` / `final_answer` / `null` | explicit phaseは実測済み、`null`はschema確認 | explicit phaseでfinal Messageとassistant detailを分離する。`null`はunknownとしてfallbackする |
...
`--experimental`付きschemaではprocess制御、Thread item / Turn一覧、remote control、一部realtime requestなどの追加APIが生成される。`codex-cli 0.144.6`では`turn/steer`、`turn/interrupt`、agentMessage phaseはstable schemaに存在した。初期Provider Adapterはstable schemaに含まれる操作だけを基準とし、experimental APIを必須機能にしない。

## 追加検証が必要な項目

- command / file / permission approval の allow、deny、timeout、重複回答
- `item/tool/requestUserInput` と MCP elicitation の回答形式
- runtime host local IPCのclient-only切断、response loss、subscription再接続、stale endpoint
- 同一 process 上の複数 Thread / Turn の event 相関と並行性
- `modelProvider/capabilities/read` の version 差分
- agentMessage `phase=null`が実際に発生するmodel / executionと複数unknown itemの順序
