...
| MT-023D10 | Workspace live Git Diff | repository root と、その下位 directory を Workspace にした Session で Files / Changes を切り替え、Working Tree と Staged の row、および通常 file preview の Open Diff を開く。Split / Inline を切り替える | Workspace 内の選択した 1 file と scope の diff だけが中央に表示され、staged / working tree の status が個別に表示される。Split が既定で、Inline と同じ patch、検索結果、Reload結果を表示する。Workspace 外の変更は混ざらず、untracked は file preview を開く。既存 chat artifact の Open Diff は従来の inline modal / Diff Window を使い、live Git Diff とは接続されない |
| MT-023D10A | Workspace Git filter guard | 対象 file に clean または process filter を設定した repository で Changes を開く | filter command は実行されず、Changes に利用できない理由が表示される |
