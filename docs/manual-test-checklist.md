...
| MT-023D6 | Central file preview / Action Dock | Text file を選び、preview を表示したまま Action Dock へ入力、添付、送信し、Back to Chat で戻る | message list だけが preview に置き換わり、Action Dock は利用できる。draft、run、chat の表示状態は戻った後も維持される |
...
| MT-023D8 | Image preview / loading | 大きい raster image と SVG を開き、Zoom Out / In、100%、Fit、Reload を操作する | local byte progress が表示され、既定は intrinsic 100%。各 zoom 操作が使え、別 file へ切り替えると前の表示と zoom state が破棄される |
...
| MT-023D10 | Workspace live Git Diff | repository root と、その下位 directory を Workspace にした Session で Files / Changes を切り替え、Working Tree と Staged の row、および通常 file preview の Open Diff を開く | Workspace 内の選択した 1 file と scope の diff だけが中央に表示され、staged / working tree の status が個別に表示される。Workspace 外の変更は混ざらず、untracked は file preview を開く。既存 chat artifact の Open Diff は従来の inline modal / Diff Window を使い、live Git Diff とは接続されない |
