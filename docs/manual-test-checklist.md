...
| MT-023D8 | Image preview / loading / copy | 大きい raster image と SVG を開き、Zoom Out / In、100%、Fit、Reload、toolbarのCopy Image、画像上の右クリックCopy Imageを操作し、画像対応アプリへ貼り付ける | local byte progress が表示され、既定は intrinsic 100%。各 zoom 操作が使え、toolbarとcontext menuの両方から表示中の画像をcopyできる。別 file へ切り替えると前の表示と zoom state が破棄される |
