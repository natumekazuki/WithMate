...
| MT-023A | Session wide layout baseline | `1920x1080` 前後の幅で Session Window を開く | 中央に `message list または preview`、上下に full-width Header / ActionDock、左右に File Explorer / Context pane の splitter が配置される。初期状態は Header hidden、ActionDock compact、side pane none になる |
...
| MT-023C | Session action dock baseline | Session Window を開き、textarea / attachment / skill / approval / model / depth / `Send` の位置関係を見る。続けて設定群が折り返す幅と最小幅まで Window を狭める | これらは full-width の下 `ActionDock` にまとまり、expanded 時だけ full editor と設定群が表示される。textarea は Send に幅を取られず横幅いっぱいに表示される。`Send` は設定群の枠外で下端が揃い、中幅では approval / model / depth が設定群内で先に折り返し、最小幅では設定群の下へ移る。`File / Folder / Image` は attachment group、`Skill` は別ボタンとして区別される |
| MT-023C1 | Session narrow layout reachability | viewport を `1400px` 未満まで狭めた Session Window を開く | Header と ActionDock は full-width の上下 dock に残り、active side pane と中央 surface がその間で縦 stack になる。`Latest Command` と provider に応じた `Tasks` / `Context` へ到達できる |
...
| MT-023D6 | Central file preview / Action Dock | Text file を選び、preview を表示したまま Action Dock へ入力、添付、送信し、Back to Chat で戻る | message list だけが preview に置き換わり、Action Dock は利用できる。draft と run の状態は維持され、戻った chat は最新メッセージが見える末尾位置で表示される |
...
| MT-023D9 | Session active-surface find / copy | 古い message を含む chat、Text、Markdown、live Git Diff で Ctrl+F、Enter、Shift+Enter、Escape を使い、Text / Markdown / diff の本文を選択する | active surface だけを検索し、未 mount の chat message / text line / diff row へ移動できる。選択時は chat と同じ floating Copy が出て、画像では出ない |
| MT-023D10 | File root live Git Diff | Workspace、非 Git の Session Folder、別 repository の Additional Directory、Workspace を含む親 Additional Directory を表示した Session で Files / Changes を切り替え、Working Tree と Staged の row、および通常 file preview の Open Diff を開く。Split / Inline を切り替える | Git root だけが Files と同じ順序で表示され、非 Git root は表示されない。包含関係にある root の同じ file は各 root の相対 path で重複表示される。選択した root、file、scope の diff だけが中央に表示され、Split が既定で Inline と同じ patch、検索結果、Reload 結果を表示する。untracked は同じ root の file preview を開く。既存 chat artifact の Open Diff は従来の inline modal / Diff Window を使い、live Git Diff とは接続されない |
...
| MT-023D11 | Path open failure feedback | chat、Markdown preview、Workspace 操作から local / external link、存在しない path、root 外へ出る相対 path、既定アプリ未関連付けの file を開く | 成功時は既定アプリまたは外部ブラウザが開く。root 外へ出る相対 path は raw target で再試行されず、typed result または例外の失敗理由が操作元で表示される。未関連付け file は Explorer へ自動で切り替わらず、失敗理由が表示される |
| MT-023E | Session action dock compact/expand | idle で draft 空の Session Window を開き、下 splitter を click / drag する | 初期状態は compact で、splitter の click で expanded / compact を切り替えられる。expanded 中は中央 surface を最低280px残したまま layout 高の40%まで広げられ、上部操作列と下部設定・送信列は固定されて textarea だけが伸縮する。dock 内に `Hide` は出ない |
