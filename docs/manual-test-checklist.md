...
| MT-024 | Live progress sort / emphasis | `in_progress` と `completed` が混在する run を実行し、可能なら `pending` または未知 status 相当の step も観察する | `Activity Monitor` で `failed / canceled / in_progress` bucket が先頭、`completed` が後段に並び、`pending` や未知 status は completed より前へ割り込まず safe degradation し、`in_progress` が最も目立つ |
| MT-025 | Live progress labels | `Activity Monitor` と assistant artifact の operation timeline を見比べる | `type` label が両方で一致し、step `status` は `実行中 / 完了 / エラー / キャンセル / 待機` の人間向け表記になる |
| MT-026 | Live progress command visibility | `command_execution` を含む run を実行し、`Activity Monitor` を assistantText 未着時と completed 直前の両方で確認する | command 文字列が常時表示され、通常 paragraph ではなく command 専用の monospace block として即判別できる |
| MT-027 | Live progress running without assistantText | step 更新が先に来て assistantText が遅れる run を観察する | pending bubble に実行中 indicator が出て、`Activity Monitor` には `in_progress` step が表示される。typing dots だけに依存せず「今動いている」と判断できる |
| MT-028 | Live progress pending indicator persistence | run 開始直後から assistantText streaming 開始後まで pending bubble を観察する。可能なら本文と `Activity Monitor` が同時に見える run も確認する | 本文が出始めても `runState === "running"` の間は先頭の実行中 indicator が残り、character 名が取れる時は `<キャラ名>が…` の形、取れない時は主語なしの一般化表現で表示される。`runState !== "running"` になった時点で indicator が消える |
| MT-029 | Live progress details / usage | command output や todo 更新を含む run を実行する | `Activity Monitor` で command 本体や主要 summary は常時表示のまま、`details` は二次情報として折りたたまれ、usage は monitor footer 集約のみで `input / output` 常時表示、`cached` は 0 より大きい時だけ表示される |
| MT-030 | Live progress assistant text separation | assistant 本文と step 更新が両方ある run を実行する | `assistantText` が pending bubble 本文として表示され、`Activity Monitor` 側へ `agent_message` を live step row として重複表示しない |
| MT-031 | Live progress file_change visibility-first | 複数ファイルを変更する run を実行し、`file_change` summary が複数行になる状態を作る | `Activity Monitor` の `file_change` step が paragraph 1 個ではなく action chip + path の line item list で表示され、monitor 自体の高さを暴れさせすぎない範囲で scan しやすい |
| MT-032 | Live progress file_change raw fallback | `file_change` summary が 1 行の run、または複数行でも `kind: path` として読みにくい summary を確認する | `Activity Monitor` で既存どおり raw summary fallback が使われ、非 `file_change` step の表示も退行しない |
| MT-033 | Live progress error block | provider error または tool error を再現する | `Activity Monitor` 内で `liveRun.errorMessage` が step list と分離した alert block に出て、failed / canceled step と見た目が混線しない |
...
| MT-035 | Live progress failed step and error block separation without assistantText | `assistantText` 未着のまま `failed` step と `liveRun.errorMessage` が同時に出る run を観察する | `Activity Monitor` で failed step は `エラー` として見え、`liveRun.errorMessage` は別 alert block に出て、pending bubble の `実行中` 表示とも競合しない |
...
| MT-036A | Activity Monitor follow mode | 実行中に `Activity Monitor` を下端から 48px 以上上へスクロールして読み返し中にし、そのまま新しい command / file change / details 更新を発生させる。続けて `最新へ` を押し、最後に session 切替か run 完了を待つ | monitor 読み返し中は位置が維持され、badge が `新着あり` または `読み返し中` に変わる。新着が来た時だけ `最新へ` が出て、押すと monitor が末尾へ戻って follow が再開する。session 切替または run 終了で monitor の follow / unread state が reset される |
