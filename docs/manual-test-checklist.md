...
| MT-018 | Audit Log | Session Window の right pane header にある `Audit Log` を押す | 1 turn 1 record の監査ログが閲覧でき、approval 表示は provider-neutral wording になる。prompt 表示は `Logical Prompt` と `Transport Payload` に分かれる |
...
| MT-023D | Session header right pane only | Session Window を開き、左右ペイン上端の配置を見る | left 側の message list は最上端から始まり、session header は right pane 上部だけに表示される |
| MT-023D1 | Session header actions always visible | Session Window の right pane header を確認する | `Rename / Audit Log / Terminal / Delete / Close` が `More` なしで常時見える。`character-update` variant では `Terminal` だけ出ない |
| MT-023D2 | Session terminal launch | Session Window の right pane header で `Terminal` を押す | session の `workspacePath` を作業ディレクトリにした外部 terminal が開く |
