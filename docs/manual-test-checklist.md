...
| MT-004 | Settings Window | Home の `Settings` を押す | 独立した `Settings Window` が開き、保存済み設定の読込完了までは loading が出る。読み込み後は `App` / `Default Microcopy` / `Coding Agent Providers` / `Diagnostics` / `Mate Reset` / `Model Catalog` が既存値で表示され、Character editor は出ない |
...
| MT-004M | Memory V6 diagnostics summary | `Settings Window` の `Diagnostics` を開き、Memory V6 summary を確認する | runtime API status、base URL、DB path、discovery file path、active binding count、provider memory binding support、managed `withmate-memory` Skill sync status、CLI Shim status、latest Memory V6 diagnostic errors が read-only で表示される。runtime API secret、binding reference、discovery document の secret 値は表示されない |
...
| MT-004O | Memory V6 unsupported provider diagnostics | binding transport 未確認の provider を含む診断 state を開発用 fixture または `memory-binding-registry` の targeted test で確認し、Settings Diagnostics の provider support 表示を見る | 未確認 provider は `unsupported` として扱われ、binding principal は作られない。UI は unsupported capability を表示しても binding reference や secret 値は表示しない |
...
| MT-017C1 | Memory V6 binding refresh on retry | Memory V6 runtime が起動している状態で Codex または Copilot session の stale thread internal retry を再現し、retry 後のturn継続、Audit Log、Diagnostics を確認する。旧binding referenceの拒否は hidden value をUI/logへ出さない方針のため `session-runtime-service.test.ts` と `copilot-adapter.test.ts` の targeted test で確認する | renderer からの再送なしで通常turnが継続し、retry後もMemory CLI利用が壊れない。旧bindingは targeted test 上で拒否され、Memory V6 API secretやbinding referenceは UI / log / diagnostics に露出しない |
| MT-017C2 | Memory V6 session isolation | Codex または Copilot の session A / B を同時に開き、それぞれのturn中に `withmate-memory context` と current Character / project target の search を実行する。別session binding reference の流用拒否は hidden value をUI/logへ出さない方針のため `memory-binding-registry.test.ts` と `memory-v6-runtime.test.ts` の targeted test で確認する | session A / B の通常turnとMemory CLI利用が混線しない。別session reference の principal 解決拒否は targeted test 上で証明され、UI / log / diagnostics にbinding referenceやsecret値は露出しない |
| MT-017C3 | Memory V6 binding revoke lifecycle | Memory V6 binding を持つ session でturnを完了し、session delete または app quit 後に新規turnやDiagnosticsを確認する。旧binding referenceの再利用拒否は hidden value をUI/logへ出さない方針のため `memory-v6-runtime.test.ts` と `session-runtime-service.test.ts` の targeted test で確認する | turn終了、session delete、app quit 相当の後も新しい通常turnでは新しいbindingでMemory CLIが使える。旧binding reference は targeted test 上で拒否され、UI / log / diagnostics に露出しない |
...
| MT-051 | `@path` keyboard navigation | query 2 文字以上で `@path` 候補を開き、`ArrowUp` / `ArrowDown` / `Enter` / `Tab` / `Escape` と mouse を試す | 候補 open 中だけ keyboard navigation が働き、basename 優先 row で active 候補が視認できる。file / folder の候補が出て、kind label と背景色で区別できる。`Enter` で候補採用、`Escape` で close、`Tab` は通常の focus 移動を優先し、候補採用には使われない。query 1 文字では候補検索を始めず、候補を閉じた通常時の textarea 操作と `Ctrl+Enter` / `Cmd+Enter` 送信は退行しない |
