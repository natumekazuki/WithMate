# Feature Guides

WithMateの利用者向け機能ガイドです。現在の設計・実装上の責務と文書の保存区分は[Documentation Map](../design/documentation-map.md)、設計判断の履歴と適用状態は[ADR](../adr/)、恒久保存するリリースごとの変更一覧は[Release Notes](../releases/README.md)を参照してください。

## Sessionと画面操作

- [Git履歴とcommit preview](git-history-and-commit-preview.md)
- [Messageの縮小表示とnavigator](session-message-collapse-and-navigation.md)
- [Messageのブックマークとnavigator絞り込み](message-bookmark-filter.md)
- [キーボードショートカット](keyboard-shortcuts.md)
- [Session Windowの復元](session-window-restore.md)
- [メッセージ画像のlightbox](message-image-lightbox.md)
- [WindowsのCopy File](windows-file-object-copy.md)
- [Prompt Template workspace](prompt-template-workspace.md)
- [Markdown表示とcode block操作](markdown-rendering-and-code-actions.md)
- [Session interfaceの変更](session-interface-refinements.md)
- [Home Session一覧のpagination](home-session-pagination.md)

## RepositoryとCharacter

- [Repository Glossary](repository-glossary.md)
- [Cross-session Affect afterglow](cross-session-affect-afterglow.md)

## 性能とruntime

- [長いSessionの性能改善](long-session-performance.md)
- [running turn開始時の永続化](running-turn-start-persistence.md)
- [Memory runtimeの複数instance discovery](memory-runtime-discovery.md)
