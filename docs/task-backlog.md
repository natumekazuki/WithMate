...
| P2 | 完了 | GitHub | [#37](https://github.com/natumekazuki/WithMate/issues/37) | セッションウインドウのヘッダー調整 | 左列を `message list + Action Dock` に使い切りつつ、右列には collapsed title handle と context pane を置く構成へ整理した | 通常時は right pane 上部に title handle だけを出し、押した時だけ full-width header を展開して `Rename / Audit Log / Terminal / Delete / Close` を常設表示する |
