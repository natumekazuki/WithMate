...
| P1 | 完了 | Local | `session-keyboard-a11y` | セッション周辺のキーボード操作基盤 | モーダル、選択系 UI、Diff 閲覧のキーボード操作を WAI-ARIA / desktop UX に沿って揃えた | `New Session` / character update / `Audit Log` / inline diff の `Escape + focus trap + 初期 focus`、provider / approval chip の矢印キー、Diff keyboard scroll へ更新済み。`@path` 入力中候補は廃止し、送信時 validation に寄せた |
