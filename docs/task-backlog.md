...
| --- | --- | --- | --- | --- | --- | --- |
| P2 | 完了 | Local | `audit-log-storage-split` | AuditLog の重い payload 外出し | `raw_items_json` や provider response が巨大化すると SQLite read / Electron IPC / renderer 表示が詰まるため、AuditLog を summary / small detail / blob payload に分割する | V3 DB / compressed blob store / SessionStorageV3 / AuditLogStorageV3 / CompanionStorageV3 / CompanionAuditLogStorageV3 / Companion runtime write path / V2 -> V3 migration / V3 blob repair report まで実装済み。DB には一覧・検索・小さい metadata を残し、raw items / full response / operation detail / artifact / diff は blob 化する。削除済み Session / Companion の audit と raw/detail blob は現方針では保持しない。IPC / renderer は Audit detail section lazy load、metadata query の summary 化、Companion merge run history summary 化、Session / Companion message artifact detail lazy load まで完了。 |
...
| P1 | 完了 | GitHub | [#24](https://github.com/natumekazuki/WithMate/issues/24) | モデル切り替えバグ | model / reasoning 変更後も会話継続を優先し、stale thread 起因の失敗だけを吸収するようにした | 通常は既存 `threadId` を維持して新設定で resume し、stale / incompatible thread の場合だけ `threadId clear + provider cache invalidate + internal retry` で新規 thread へ回復する |
...
| P1 | 完了 | GitHub | [#3](https://github.com/natumekazuki/WithMate/issues/3) | Memory 永続化と共有 | current scope の Project / Session / Character Memory 永続化、抽出 plane、retrieval 基盤は実装済み | `Session / Project / Character Memory` の persistence、`Session -> Project` 昇格、coding plane への retrieval、`Character Memory` 保存 / ranking、`character reflection cycle`、管理 UI まで完了。将来の共有拡張は reopen または別 issue で扱う |
...
| P1 | 完了 | GitHub | [#25](https://github.com/natumekazuki/WithMate/issues/25) | 独り言生成タイミング | session を開くたびに独り言を生成せず、main chat と monologue の新しさで trigger したい | `SessionStart` は「前回 reflection 以降に会話増分がある時だけ」へ修正済み。通常 turn 後の `context-growth` は app-wide settings から `cooldown / char delta / message delta` を調整できるようにした |
| P1 | 完了 | Local | `session-keyboard-a11y` | セッション周辺のキーボード操作基盤 | モーダル、選択系 UI、Diff 閲覧のキーボード操作を WAI-ARIA / desktop UX に沿って揃えた | `New Session` / Character authoring / `Audit Log` / inline diff の `Escape + focus trap + 初期 focus`、provider / approval chip の矢印キー、Diff keyboard scroll へ更新済み。`@path` 入力中候補は廃止し、送信時 validation に寄せた |
...
| P2 | 完了 | GitHub | [#17](https://github.com/natumekazuki/WithMate/issues/17) | `tasks` コマンドの SDK 調査と実装 | Copilot SDK は `session.idle.backgroundTasks` と `system.notification` で background task snapshot を扱えることを確認し、Session 右ペインの Copilot 専用 `Tasks` tab として実装した | current scope は Copilot-only。Codex SDK `@openai/codex-sdk@0.114.0` には同等 surface が見えないため parity は見送り、`docs/design/coding-agent-capability-matrix.md` と `docs/design/provider-sdk-pending-items.md` を正本にする |
| P2 | 完了 | GitHub | [#28](https://github.com/natumekazuki/WithMate/issues/28) | データ export / import | 少なくともキャラ定義の移行は専用 export / import を作らず、character directory のファイルコピーで対応する | current scope では Memory 同期は対象外。character の正本は filesystem 配下なので、`userData/characters/<character-id>/` のコピーで持ち運べる |
| P2 | 完了 | GitHub | [#26](https://github.com/natumekazuki/WithMate/issues/26) | ウインドウ生成場所 | `Home Window` 以外の新規 window を cursor がある display の workArea 起点で開くようにした | 既存 window 再利用時は移動せず focus のみ。placement clamp は helper と test に分離し、window lifecycle doc と manual checklist を同期した |
| P2 | 完了 | GitHub | [#23](https://github.com/natumekazuki/WithMate/issues/23) | `**message**` markdown 未反映 | `MessageRichText` の inline parser を見直し、`**bold**` を strong として render できるようにした | code / link / path 導線は維持し、renderer test を追加して `code + link + bold` の併用も固定した |
...
| P3 | 完了 | GitHub | [#4](https://github.com/natumekazuki/WithMate/issues/4) | キャラ定義の自己改善 | `character-authoring` Session で `character.md` / `character-notes.md` を更新できる導線を実装済み | Character directory、通常 composer、app 管理の固定 Skill を使う。自動改善 loop は対象外 |
...
1. `#3 Memory 永続化と共有`
2. `#41 チャットの Details ボタンが邪魔`
3. `#10 Copilot custom slash command`
4. `#17 tasks コマンドの SDK 調査と実装`
5. `#23 **message** markdown 未反映`
