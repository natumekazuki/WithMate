...
| --- | --- | --- | --- | --- | --- | --- |
| P1 | 未着手 | GitHub | [#24](https://github.com/natumekazuki/WithMate/issues/24) | モデル切り替えバグ | model を変えると `セッションが存在しない` エラーで継続できない | `#32` と同じ session resume / provider thread lifetime クラスタで、model switch 時の stale `threadId` reuse が露出している可能性が高い。加えて Codex では model-switch resume 非互換も候補 |
| P1 | 未着手 | GitHub | [#32](https://github.com/natumekazuki/WithMate/issues/32) | 長時間放置後の Session not found | 最終リクエストから約 1 時間空いた session が再開時に NotFound になる原因を調査し、失効検知と再接続 / 復旧方針を入れる | long-idle 後の expiry / recovery 不足の基準ケース。`#24` と同じ session resume / provider thread lifetime クラスタとして扱いつつ、model-switch 固有要因と切り分けて実測する |
