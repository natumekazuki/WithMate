...
| P2 | 完了 | GitHub | [#37](https://github.com/natumekazuki/WithMate/issues/37) | セッションウインドウのヘッダー調整 | header 自体を right pane 上部へ移し、左ペインの chat 面が最上端から始まるようにした | `More` は撤去し、`Rename / Audit Log / Terminal / Delete / Close` を right pane header に常設した |
