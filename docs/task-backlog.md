...
| P2 | 完了 | GitHub | [#37](https://github.com/natumekazuki/WithMate/issues/37) | セッションウインドウのヘッダー調整 | `Audit Log` と `Terminal` を right pane の utility action へ寄せ、Top Bar は `title / More / Close` 中心へ整理した | `Rename / Delete` は従来どおり `More` 配下に残し、左ペインは chat と `Action Dock` に集中できるようにした |
