...
| --- | --- | --- | --- | --- | --- | --- |
| P2 | 未着手 | Local | `audit-log-storage-split` | AuditLog の重い payload 外出し | `raw_items_json` や provider response が巨大化すると SQLite read / Electron IPC / renderer 表示が詰まるため、AuditLog を summary / small detail / blob payload に分割する | 今回の Companion AuditLog 追加とは別タスク。DB には一覧・検索・小さい metadata を残し、raw items / full response / operation detail は file blob 化または section 別 lazy load にする。現行の `audit_logs.session_id` は `sessions(id)` 参照なので、Companion の実保存対応と削除済み session の audit 保持方針も同時に再検討する |
