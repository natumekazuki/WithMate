...
// @test-value v2
// kind = "invariant"
// claim = "Reasoning live detailsの本文preは外側のdetails領域へ委譲し、内側scrollを作らない"
// oracle = { type = "contract", ref = "session context reasoning details layout" }
// fault = "reasoning本文が内側に固定高scrollを持ち、長文の閲覧領域を二重scrollにする"
// observable = "live reasoning detailsのcomponent classとpreのmax-height、overflow CSS宣言"
// observation_boundary = "declaration"
// scope = "session-context-reasoning-details-css"
// lifecycle = "permanent"
// distinction = "reasoning内容やstream状態ではなく、本文scroll ownershipのCSS契約を確認する"
// @end-test-value
test("Reasoning live details は内側 pre スクロールを作らない", async () => {
  const [componentSource, stylesSource] = await Promise.all([
    readFile("src/chat/shell/session-context-pane.tsx", "utf8"),
    readStylesheet(),
  ]);
...
// @test-value v2
// kind = "invariant"
// claim = "Command detailsは長いcommandとsingle-line resultをdetails内で折り返す"
// oracle = { type = "contract", ref = "session context command details layout" }
// fault = "commandまたはresultがnowrapのまま横へ溢れ、detailsの可読範囲を超える"
// observable = "command text、summary、preのmin-width、wrap、word-break CSS宣言"
// observation_boundary = "declaration"
// scope = "session-context-command-details-css"
// lifecycle = "permanent"
// distinction = "command実行結果や文字列変換ではなく、長文表示のwrap CSS契約を確認する"
// @end-test-value
...
// @test-value v2
// kind = "invariant"
// claim = "Artifact result detailsは長いpathと本文を折り返して表示する"
// oracle = { type = "contract", ref = "session context artifact result layout" }
// fault = "artifact pathまたは本文が横溢れし、結果詳細を閲覧できない"
// observable = "artifact meta codeとoperation本文要素のwhite-space、wrap、word-break CSS宣言"
// observation_boundary = "declaration"
// scope = "session-context-artifact-result-css"
// lifecycle = "permanent"
// distinction = "artifact生成やpath解決ではなく、結果表示のoverflow containmentを確認する"
// @end-test-value
...
// @test-value v2
// kind = "invariant"
// claim = "Session header menuはHeader Dockの固定高にclipされず中央領域へ重ねて表示できる"
// oracle = { type = "contract", ref = "session header menu overlay layout" }
// fault = "header dockのoverflowでmenuが切り取られ、操作項目へ到達できない"
// observable = "header dock slotのposition、z-index、overflow CSSとaction dock slotのoverflow宣言"
// observation_boundary = "declaration"
// scope = "session-header-menu-overlay-css"
// lifecycle = "permanent"
// distinction = "menuの開閉状態やpointer操作ではなく、overlayを許容するstacking/overflow境界を確認する"
// @end-test-value
...
// @test-value v2
// kind = "invariant"
// claim = "selection action overlayはSession layout内でsurfaceより前、modalより後ろのstacking層を所有する"
// oracle = { type = "contract", ref = "session selection action overlay stacking" }
// fault = "overlayがmodalに隠れる、または通常surfaceの背面へ回り選択操作を失う"
// observable = "session layout isolation、overlay fixed/z-index/pointer-events、message actionのinteractive宣言"
// observation_boundary = "declaration"
// scope = "session-selection-action-overlay-css"
// lifecycle = "permanent"
// distinction = "selection stateやmodal実装ではなく、CSS stacking contextとpointer boundaryを確認する。overlayのz-indexがmodalより低いことはこのrecordで直接確認し、modalの固定値自体はvertical track recordでも確認する"
// @end-test-value
...
test("wide layout はHeaderとActionDockを全幅dockとして表示する", async () => {
  const [componentSource, stylesSource] = await Promise.all([
    readFile("src/chat/shell/session-header.tsx", "utf8"),
    readStylesheet(),
  ]);
...
// @test-value v2
// kind = "invariant"
// claim = "固定高のSession Headerは操作群を折り返さず、タイトルを残り幅へ収める"
// oracle = { type = "contract", ref = "session header responsive layout" }
// fault = "header操作群が折り返して高さを増やす、またはtitleが操作群へ重なる"
// observable = "top bar container、window controlsのflex、titleのflex、narrow container CSS宣言"
// observation_boundary = "declaration"
// scope = "session-header-responsive-css"
// lifecycle = "permanent"
// distinction = "header control stateや文字列ではなく、固定高と残余幅のCSS契約を確認する"
// @end-test-value
...
// @test-value v2
// kind = "invariant"
// claim = "Session四辺の開閉は共通motionを使い、resize中とreduced-motionでは補間を停止する"
// oracle = { type = "contract", ref = "session dock motion accessibility" }
// fault = "resize操作中またはreduced-motion設定下でもtransitionが走り、操作追従や利用者設定を損なう"
// observable = "session dockのtransition、resizing state、prefers-reduced-motion CSS宣言"
// observation_boundary = "declaration"
// scope = "session-dock-motion-css"
// lifecycle = "permanent"
// distinction = "resize hookの数値計算ではなく、共通motionと停止条件のCSS境界を確認する"
// @end-test-value
...
// @test-value v2
// kind = "invariant"
// claim = "file previewはfindやfeedbackの有無にかかわらず本文を固定scroll rowへ配置する"
// oracle = { type = "contract", ref = "session file preview layout" }
// fault = "補助UIの有無で本文rowが伸縮し、preview本文のscroll ownershipが変わる"
// observable = "file previewのgrid row、scroll、条件付きfind/feedback領域CSS宣言"
// observation_boundary = "declaration"
// scope = "session-file-preview-scroll-layout-css"
// lifecycle = "permanent"
// distinction = "find状態やfeedback生成ではなく、本文rowの固定とscroll境界を確認する"
// @end-test-value
test("file preview は条件付き find / feedback の有無にかかわらず本文を固定 scroll row に置く", async () => {
  ...
  assert.match(
    stylesSource,
    /\.session-file-text-scroll,\s*\.session-file-markdown-scroll,\s*\.session-file-image-scroll\s*{\s*overflow:\s*auto;\s*}/,
  );
  assert.match(
    stylesSource,
...
// @test-value v2
// kind = "invariant"
// claim = "Markdown previewはchat本文と同じline-heightとblock間隔で表示する"
// oracle = { type = "contract", ref = "markdown preview typography" }
// fault = "previewだけ本文行間または段落間隔が異なり、chatとの読み取りリズムが崩れる"
// observable = "rich-text、session-file-markdown、message paragraphのdisplay、gap、line-height CSS宣言"
// observation_boundary = "declaration"
// scope = "markdown-preview-typography-css"
// lifecycle = "permanent"
// distinction = "Markdown変換結果ではなく、chatとpreviewで共有する本文typography契約を確認する"
// @end-test-value
...
// @test-value v2
// kind = "invariant"
// claim = "Markdown headingは階層を、thematic breakは本文全幅の区切りを視認できる"
// oracle = { type = "contract", ref = "markdown preview heading and divider styles" }
// fault = "見出し階層が同じ大きさになる、または区切りが本文幅を満たさず構造を示せない"
// observable = "heading levelごとのfont-sizeとdividerのwidth、border CSS宣言"
// observation_boundary = "declaration"
// scope = "markdown-preview-heading-divider-css"
// lifecycle = "permanent"
// distinction = "Markdown ASTの見出し生成ではなく、階層と区切りの視覚表現契約を確認する"
// @end-test-value
...
// @test-value v2
// kind = "invariant"
// claim = "Markdown listはlogical paddingと階層ごとのmarkerで入れ子構造を示す"
// oracle = { type = "contract", ref = "markdown preview list styles" }
// fault = "listの方向依存paddingやnested markerが失われ、階層を識別できない"
// observable = "listのpadding、position、nested list marker、marker color CSS宣言"
// observation_boundary = "declaration"
// scope = "markdown-preview-list-hierarchy-css"
// lifecycle = "permanent"
// distinction = "list parserのnesting結果ではなく、logical layoutとmarker表現を確認する"
// @end-test-value
...
// @test-value v2
// kind = "invariant"
// claim = "Markdown listはitem間の改行を余白にせずnative markerと本文を同じblock flowに置く"
// oracle = { type = "contract", ref = "markdown preview list flow" }
// fault = "list itemがgrid等で余計な行間を作り、native markerと本文のblock flowが分離する"
// observable = "list display、white-space、item間margin、nested list margin CSS宣言"
// observation_boundary = "declaration"
// scope = "markdown-preview-list-flow-css"
// lifecycle = "permanent"
// distinction = "Markdown itemの生成順ではなく、CSS block flowと余白の契約を確認する"
// @end-test-value
...
// @test-value v2
// kind = "invariant"
// claim = "Markdown previewは中央本文を固定幅にせず利用可能なcontent領域を使う"
// oracle = { type = "contract", ref = "markdown preview content width" }
// fault = "preview本文が固定幅や不要なmarginで狭まり、広いcontent領域を利用できない"
// observable = "markdown scroll paddingとmarkdown width、max-width、margin CSS宣言"
// observation_boundary = "declaration"
// scope = "markdown-preview-content-width-css"
// lifecycle = "permanent"
// distinction = "本文内容やviewport測定ではなく、content領域を使うCSS width契約を確認する"
// @end-test-value
