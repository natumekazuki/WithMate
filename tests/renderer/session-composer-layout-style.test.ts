...
// observable = "src/chat/composer/session-composer.tsxの6設定field selectorとreadStylesheet()で展開したentryの設定field宣言・container breakpoint宣言"
...
test("Session composer は設定field内を一行にし、通常幅で設定群を保ち、狭幅で折り返す", async () => {
  ...
  const sessionComponentsSource = await readFile("src/chat/composer/session-composer.tsx", "utf8");
...
// observable = "readStylesheet()で展開したentryのexpanded Send最小幅、Cancel slot固定幅・不可視・狭幅上書き宣言"
// observation_boundary = "declaration"
