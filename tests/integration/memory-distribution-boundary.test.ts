...
} from "../../src-electron/skills/managed-skill-distribution-service.js";
...
describe("Memory distribution boundary", () => {
  ...
  // fault = "upgradeまたはSettings操作がprovider側の既存withmate-memory Skill directoryを検査、更新、削除する"
  // observable = "Memory Skill pathへのaccess失敗とmain・distribution sourceのmanaged memory wiring不在"
  // observation_boundary = "declaration"
  // scope = "withmate-memory-distribution-stop"
  // lifecycle = "permanent"
  // impact = "Settingsまたはupgradeがprovider所有のlegacy Skillを変更すると、利用者の既存workflowとprovider管理境界を破壊する"
  // distinction = "source declarationの旧Memory wiring不在と、実際のprovider Skill sentinel非変更を同時に確認し、通常のtype/buildでは検出できない境界を検証する"
  // @end-test-value
  it("Memory Skill catalogとmanaged sync入口をrepositoryから除外する", async () => {
    const mainSource = await readFile("src-electron/main.ts", "utf8");
    const distributionSource = await readFile("src-electron/skills/managed-skill-distribution-service.ts", "utf8");
