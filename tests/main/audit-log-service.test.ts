...
test("AuditLogService は storage の CRUD を委譲する", async () => {
  ...
  assert.deepEqual(await service.createAuditLog(createInputValue), createdEntry);
  const updateInputValue = createInput({ phase: "completed" });
  assert.deepEqual(await service.updateAuditLog(1, updateInputValue), updatedEntry);
