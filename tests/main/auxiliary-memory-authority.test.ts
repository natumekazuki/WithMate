...
import { buildProviderAgentRuntimeAuthoritySnapshot } from "../../src-electron/providers/provider-agent-runtime-binding.js";
...
import { createMemoryV6HttpServer, getMemoryV6AgentRuntimeOperations } from "../../src-electron/memory/memory-v6-http-server.js";
...
it("Auxiliary runtime sessionからMemory public actor authorityをowner別に解決する", async () => {
  ...
  const service = new AuxiliarySessionService({
    ...
    getStorage: () => ({
      ...
      updateAuxiliarySessionIfMatches: () => null,
      ...
      saveAuxiliaryDraft: () => ({ outcome: "saved" }),
      consumeAuxiliaryDraft: () => ({ outcome: "consumed" }),
