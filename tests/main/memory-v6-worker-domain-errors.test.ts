...
import { MemoryV6FileQuotaExceededError, MemoryV6IdempotencyConflictError, type MemoryV6Storage } from "../../src-electron/memory/memory-v6-storage.js";
import { CharacterAffectIdempotencyConflictError, CharacterAffectVersionConflictError } from "../../src-electron/character/character-affect-storage.js";
...
// oracle = { type = "contract", ref = "src-electron/storage/storage-worker-dispatcher.ts; src-electron/storage/storage-worker-client.ts; src-electron/memory/memory-v6-worker-handlers.ts" }
...
test("Memory Worker preserves quota and idempotency rejection types", async () => {
  ...
  try {
    ...
    worker = new MemoryV6WorkerClient({ dbPath, workerUrl: new URL("../../src-electron/storage/storage-worker-entry.ts", import.meta.url) });
...
// oracle = { type = "contract", ref = "src-electron/storage/storage-worker-dispatcher.ts; src-electron/storage/storage-worker-client.ts; src-electron/character/character-affect-storage.ts" }
