...
} from "../../src-electron/platform/memory/memory-discovery.js";
import {
  buildRuntimeDiscoveryCredentialFileName,
} from "../../src-electron/platform/runtime-discovery/runtime-discovery-contract.js";
import {
  listRuntimeDiscoveryRegistryEntries,
  readRuntimeDiscoveryCredential,
  resolveRuntimeDiscoveryMutationLockFilePath,
} from "../../src-electron/platform/runtime-discovery/runtime-discovery-registry.js";
import {
...
} from "../../src-electron/memory/memory-v6-runtime.js";
import {
...
describe("Memory V6 runtime API", () => {
  // @test-value v2
  // kind = "security"
  // claim = "preflight成功後の不正headerによるHTTP request生成失敗を、dispatched=falseのexchange errorへ正規化する"
  // oracle = { type = "contract", ref = "src-cli/memory/withmate-memory-runtime-client.ts: callWithMateMemoryRuntime / WithMateMemoryRuntimeExchangeError" }
  // fault = "HTTP request生成失敗をdispatch済みとして扱う、原因を失う、またはpreflight URLへbodyやapiSecretを混入させる"
  // observable = "exchange errorのmessage、dispatched=false、causeのERR_INVALID_CHAR、preflight request数1、preflight URLのbody/API secret非混入"
  // observation_boundary = "public-boundary"
  // scope = "memory-v6-runtime-transport"
  // lifecycle = "permanent"
  // @end-test-value
  it("不正headerによるrequest生成失敗をpre-dispatch exchange errorへ正規化する", async () => {
    const preflightRequests: string[] = [];
    const apiSecret = "api-secret";
    const runtimeGenerationId = "invalid\ngeneration";
    await assert.rejects(
      () => callWithMateMemoryRuntime({
        api: {
          ...
          apiSecret,
          runtimeGenerationId,
          runtimeInstanceId: "runtime-generation",
        },
        ...
        fetch: async (input) => {
          ...
          const challenge = createHmac("sha256", apiSecret).update(nonce, "utf8").digest("base64url");
          return new Response(JSON.stringify({
            runtimeInstanceId: runtimeGenerationId,
            runtimeGenerationId,
            challenge: { nonce, hmacSha256: challenge },
          }), { status: 200, headers: { "content-type": "application/json" } });
        },
      }),
      (error: unknown) => {
        assert.ok(error instanceof WithMateMemoryRuntimeExchangeError);
        assert.equal(error.dispatched, false);
        assert.equal(error.message, "Memory API request could not be created.");
        assert.equal((error.cause as NodeJS.ErrnoException | undefined)?.code, "ERR_INVALID_CHAR");
        return true;
      },
    );
    assert.equal(preflightRequests.length, 1);
    assert.equal(preflightRequests[0]?.includes("must-not-be-dispatched"), false);
    assert.equal(preflightRequests[0]?.includes(apiSecret), false);
  });
