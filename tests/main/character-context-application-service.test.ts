...
} from "../../src-electron/character/character-context-application-service.js";
import { CharacterAffectService } from "../../src-electron/character/character-affect-service.js";
import { createCharacterAffectServiceWithMemory } from "../../src-electron/character/character-affect-memory-adapter.js";
...
describe("CharacterContextApplicationService", () => {
  ...
  // fault = "principalを欠落または別binding namespaceへ写像し、同一session内の同一keyを衝突させる"
  ...
  it("Character Memory appendはbinding principalをsourceとidempotency namespaceへ引き渡す", async () => {
    ...
    try {
      ...
      try {
        ...
        assert.deepEqual(rows.map((row) => row.source_session_id), ["session-a", "session-a"]);
        const namespaces = db.prepare(`
          SELECT binding_id_hash, key, response_entry_id
          FROM memory_idempotency_keys_v6
          WHERE operation = 'append' AND key = ?
          ORDER BY binding_id_hash
        `).all("same-key") as Array<{
          binding_id_hash: string;
          key: string;
          response_entry_id: string;
        }>;
        assert.deepEqual(namespaces.map((row) => ({
          binding_id_hash: row.binding_id_hash,
          key: row.key,
          response_entry_id: row.response_entry_id,
        })), [
          { binding_id_hash: "binding-a", key: "same-key", response_entry_id: first.entry.id },
          { binding_id_hash: "binding-b", key: "same-key", response_entry_id: second.entry.id },
        ]);
      } finally {
        db.close();
      }
