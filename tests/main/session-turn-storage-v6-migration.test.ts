...
describe("session turn storage v6 migration dry-run", () => {
  ...
  // oracle = { type = "contract", ref = "src-electron/storage/migrations/migrate-session-turn-storage-v6.ts" }
  ...
  it("write mode は未移行の non-session_turn audit row がある場合も valid row を非破壊移行する", async () => {
    ...
    try {
      ...
      try {
        ...
        const retainedAuditRow = migratedDb.prepare(`
          SELECT event_type, provider_id, summary, metadata_json
          FROM audit_events_v6
          WHERE session_id = ? AND event_type = 'diagnostic'
        `).get("session-1") as {
          event_type: string;
          provider_id: string;
          summary: string;
        ...
        assert.deepEqual(retainedAuditRow && {
          event_type: retainedAuditRow.event_type,
          provider_id: retainedAuditRow.provider_id,
          summary: retainedAuditRow.summary,
          metadata_json: retainedAuditRow.metadata_json,
        }, {
