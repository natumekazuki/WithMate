...
import { ProjectMemoryStorage } from "../../src-electron/memory/project-memory-storage.js";
import { resolveProjectScope } from "../../src-electron/platform/project-scope.js";
...
describe("ProjectMemoryStorage", () => {
  ...
  it("management page query は category / search / sort / cursor / total を DB 側で適用する", async () => {
    ...
    const newWorkspace = path.join(tempDirectory, "new");
    let storage: ProjectMemoryStorage | null = null;
    let sessionStorage: SessionStorage | null = null;
    ...
    try {
      ...
      sessionStorage = new SessionStorage(dbPath);
      storage = new ProjectMemoryStorage(dbPath);
      const oldScope = storage.ensureProjectScope(resolveProjectScope(oldWorkspace));
      const newScope = storage.ensureProjectScope(resolveProjectScope(newWorkspace));
      storage.upsertProjectMemoryEntry({
        id: "entry-a",
        projectScopeId: oldScope.id,
        sourceSessionId: null,
        category: "decision",
        title: "old",
        detail: "",
      ...
      try {
        db.prepare("UPDATE project_memory_entries SET updated_at = ? WHERE id IN ('entry-a', 'entry-b')")
          .run("2026-04-02T10:00:00.000Z");
      } finally {
        db.close();
      }

      const page = storage.listProjectMemoryPage({
